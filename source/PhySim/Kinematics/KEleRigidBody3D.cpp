...
void KEleRigidBody3D::Initialize() {
  ...
  if (!this->m_pGeometry->Traits().HasTrait(Tag_Position_X))
    this->m_pGeometry->Traits().AddTrait(
        Tag_Position_X, this->m_pGeometry->Centroid(Tag::Tag_Position_X));

  if (!this->m_pGeometry->Traits().HasTrait(Tag_Position_0))
    this->m_pGeometry->Traits().AddTrait(
        Tag_Position_0, this->m_pGeometry->Centroid(Tag::Tag_Position_0));
  ...
  if (!this->m_pGeometry->Traits().HasTrait(Tag_Rotat_X))
    this->m_pGeometry->Traits().AddTrait(Tag_Rotat_X,
