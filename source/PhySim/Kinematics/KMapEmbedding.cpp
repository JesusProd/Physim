...
namespace PhySim
{
	...
	KMapEmbedding::KMapEmbedding(Simulable* pModel, PtrS<KEleParticle3D>& pParticle, PtrS<Embedding>& pEmbedding)
	{
		this->Init(pModel, pParticle, pEmbedding);
	}

	void KMapEmbedding::Init(Simulable* pModel, PtrS<KEleParticle3D>& pParticle, PtrS<Embedding>& pEmbedding)
	{
		...
		for (int i = 0; i < N; ++i)
			vIn[i].reset(pEmbedding->Master()->Nodes()[i]->Traits().Kinematics(Tag_DOF_0));
		vOut[0] = pParticle;
	...
	void KMapEmbedding::MapValue(int idxIn, const VectorXd& vpIn, int idxOut, VectorXd& vpOut)
	{
