...
namespace PhySim
{
	...
	void KinematicsEle::GatherMapChain(int owner, vector<vector<PtrS<KMappingBlock>>>& vpmBlocks, vector<KinematicsEle*>& vpKinem)
	{
		...
		else
		{
			...
			{
				assert(PortsIn().size() == 1 && "Number of inputs of an element should be mostly 1");
				PortsIn()[0]->m_pMap->GatherMapChain(owner, PortsIn()[0]->m_index, vpmBlocks, vpKinem);
			}
		}
	}

	void KinematicsEle::MapDerivatives(int owner, const MatrixXd& mBIn, vector<KinematicsEle*>& vpKOut, vector<MatrixXd>& vmBOut)
	{
		if (owner == this->Layer() && this->Active())
		{
		...
		else
		{
			if (this->HasMappingIn())
			{
				...
				this->PortsIn()[0]->m_pMap->MapDerivatives(owner,
														   PortsIn()[0]->m_index,
														   mBIn, vpKOut, vmBOut);
			}
		}
