...
KMapIdentity::KMapIdentity(Simulable* pModel,
                           PtrS<KinematicsEle>& pEleIn,
                           PtrS<KinematicsEle>& pEleOut) {
  this->Init(pModel, pEleIn, pEleOut);
}

void KMapIdentity::Init(Simulable* pModel,
                        PtrS<KinematicsEle>& pEleIn,
                        PtrS<KinematicsEle>& pEleOut) {
...
void KMapIdentity::UpdateMapPartials() {
  this->m_vDoutDin[0][0] =
      MatrixXd::Identity(this->m_vOut[0]->m_numDim, this->m_vIn[0]->m_numDim);
  this->m_isDirty = false;
}

void KMapIdentity::MapValue(int idxIn,
                            const VectorXd& vpIn,
                            int idxOut,
                            VectorXd& vpOut) {
  vpOut = vpIn;
...
void KMapIdentity::MapDerivatives(int owner,
                                  int idxOut,
                                  const MatrixXd& mBIn,
