...
void KinematicsMap::Init(Simulable* pModel,
  ...
  for (size_t iOut = 0; iOut < vOut.size(); ++iOut) {
    ...
    for (size_t iIn = 0; iIn < vIn.size(); ++iIn) {
      this->m_vDoutDin[iOut][iIn] =
          MatrixXd::Zero(vOut[iOut]->NumDim(), vIn[iIn]->NumDim());
    }
  }
...
void KinematicsMap::GatherMapChain(
    int owner,
    int idxOut,
...
void KinematicsMap::MapDerivatives(int owner,
                                   int idxOut,
                                   const MatrixXd& mBIn,
  ...
  // For each input to this kinematics map we map the input block and forward
  // the signal

  for (int i = 0; i < (int)m_vIn.size(); ++i) {
    assert(mBIn.cols() == this->m_vDoutDin[idxOut][i].rows());

    m_vIn[i]->m_pEle->MapDerivatives(
        owner, mBIn * (this->m_vDoutDin[idxOut][i]), vpKOut, vmBOut);
  }
}
}  // namespace PhySim
