...
namespace PhySim
{
	...
	{
		return PtrS<KinematicsEle>(new KEleParticle3D(this->m_pModel, this->m_pGeometry));
	}
	...
	void KEleParticle3D::Initialize()
	{
		...
		if (!this->m_pGeometry->Traits().HasTrait(Tag_Position_0))
			this->m_pGeometry->Traits().AddTrait(Tag_Position_0, this->m_pGeometry->Traits().Vector3d(Tag_Position_X));
	}

	int KEleParticle3D::GetFullStateSize() const { return 6; };
