...
void KEleDERMatTwist::Initialize() {
  if (!this->m_pGeometry->Traits().HasTrait(Tag::Angle_X))
    this->m_pGeometry->Traits().AddTrait(Tag::Angle_X, 0.0);

  if (!this->m_pGeometry->Traits().HasTrait(Tag::Angle_0))
    this->m_pGeometry->Traits().AddTrait(Tag::Angle_0, 0.0);

  if (!this->m_pGeometry->Traits().HasTrait(Tag::Angle_V))
    this->m_pGeometry->Traits().AddTrait(Tag::Angle_V, 0.0);

  if (!this->m_pGeometry->Traits().HasTrait(Tag::Frame_X))
    this->m_pGeometry->Traits().AddTrait(Tag::Frame_X, Frame3d());

  if (!this->m_pGeometry->Traits().HasTrait(Tag::Frame_0))
    this->m_pGeometry->Traits().AddTrait(Tag::Frame_0, Frame3d());
}
...
VectorXd KEleDERMatTwist::GetFullState() const {
  ...
  Frame3d F = this->m_pGeometry->Traits().Frame3d(Tag::Frame_X);
...
VectorXd KEleDERMatTwist::GetPositionX() const {
  VectorXd vx(1);
  vx[0] = this->m_pGeometry->Traits().Double(Tag::Angle_X);
  return vx;
}

VectorXd KEleDERMatTwist::GetPosition0() const {
  VectorXd vx(1);
  vx[0] = this->m_pGeometry->Traits().Double(Tag::Angle_0);
  return vx;
}

VectorXd KEleDERMatTwist::GetVelocity() const {
  VectorXd vx(1);
  vx[0] = this->m_pGeometry->Traits().Double(Tag::Angle_V);
  return vx;
}
...
void KEleDERMatTwist::SetFullState(const VectorXd& vs) {
  ...
  this->m_pGeometry->Traits().Frame3d(Tag::Frame_X) = F;
}

void KEleDERMatTwist::SetPositionX(const VectorXd& vp) {
  this->m_pGeometry->Traits().Double(Tag::Angle_X) = vp[0];
}

void KEleDERMatTwist::SetPosition0(const VectorXd& vp) {
  this->m_pGeometry->Traits().Double(Tag::Angle_0) = vp[0];
}

void KEleDERMatTwist::SetVelocity(const VectorXd& vp) {
  this->m_pGeometry->Traits().Double(Tag::Angle_V) = vp[0];
}

bool KEleDERMatTwist::UpdateKinematics() {
  this->m_pMeshDER->AdaptReferenceFrame(this->m_pGeometry->ID(), Tag::Frame_X);

  return true;
}

void KEleDERConnRot::Initialize() {
  if (!this->m_pGeometry->Traits().HasTrait(Tag::Euler_X))
    this->m_pGeometry->Traits().AddTrait(Tag::Euler_X, Vector3d::Zero());

  if (!this->m_pGeometry->Traits().HasTrait(Tag::Euler_V))
    this->m_pGeometry->Traits().AddTrait(Tag::Euler_V, Vector3d::Zero());

  if (!this->m_pGeometry->Traits().HasTrait(Tag::Rotat_X))
    this->m_pGeometry->Traits().AddTrait(Tag::Rotat_X, Matrix3d::Identity());
}
...
VectorXd KEleDERConnRot::GetFullState() const {
  ...
  Matrix3d R = this->m_pGeometry->Traits().Matrix3d(Tag::Rotat_X);
...
VectorXd KEleDERConnRot::GetPositionX() const {
  return this->m_pGeometry->Traits().Vector3d(Tag::Euler_X);
}

VectorXd KEleDERConnRot::GetVelocity() const {
  return this->m_pGeometry->Traits().Vector3d(Tag::Euler_V);
}
...
void KEleDERConnRot::SetFullState(const VectorXd& vs) {
  ...
  this->m_pGeometry->Traits().Matrix3d(Tag::Rotat_X) = R;
}

void KEleDERConnRot::SetPositionX(const VectorXd& vp) {
  this->m_pGeometry->Traits().Vector3d(Tag::Euler_X) = vp;
}

void KEleDERConnRot::SetVelocity(const VectorXd& vp) {
  this->m_pGeometry->Traits().Vector3d(Tag::Euler_V) = vp;
}
...
void KEleDERRefTwist::Initialize() {
  if (!this->m_pGeometry->Traits().HasTrait(Tag::Twist_X))
    this->m_pGeometry->Traits().AddTrait(Tag::Twist_X, 0.0);
}
...
VectorXd KEleDERRefTwist::GetPositionX() const {
  VectorXd vx(1);
  vx[0] = this->m_pGeometry->Traits().Double(Tag::Twist_X);
  return vx;
}
...
void KEleDERRefTwist::SetPositionX(const VectorXd& vp) {
  this->m_pGeometry->Traits().Double(Tag::Twist_X) = vp(0);
}
