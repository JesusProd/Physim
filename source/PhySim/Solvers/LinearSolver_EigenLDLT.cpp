...
namespace PhySim
{
	...
	LSResult LinearSolver_EigenLDLT::SolveInternal(MatrixSd& mA, const VectorXd& vb, VectorXd& vx)
	{
		int N = (int) mA.rows();
		int M = (int) mA.cols();
		...
		for (i = 0; i < this->m_options.regIters; ++i)
		{
			...
			IOUtils::logTrace(Verbosity::V1_Default, "\n[SUCCESS] Linear solve: solved using Eigen LDLT. Reg: %d. Error: %.9e", i, relError);
	...
	LSResult LinearSolver_EigenLDLT::SolveInternal(MatrixSd& mA, const MatrixXd& mB, MatrixXd& mX)
	{
	...
	LSResult LinearSolver_EigenLDLT::SolveInternal(MatrixSd& mA, const MatrixSd& mB, MatrixSd& mX)
	{
		throw exception("Not implemented");
