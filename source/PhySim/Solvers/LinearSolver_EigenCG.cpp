...
LinearSolver_EigenCG::LinearSolver_EigenCG(const MatrixSd& mA,
...
void LinearSolver_EigenCG::Init(const MatrixSd& mA,
                                const LinearSolverOptions& options) {
...
LSResult LinearSolver_EigenCG::SolveInternal(MatrixSd& mA,
                                             const VectorXd& vb,
                                             VectorXd& vx) {
  int N = (int)mA.rows();
  int M = (int)mA.cols();
  ...
  for (i = 0; i < this->m_options.regIters; ++i) {
    ...
    IOUtils::logTrace(Verbosity::V1_Default,
                      "\n[SUCCESS] Linear solve: solved using Eigen CG. Reg: "
                      "%d, Iter: %d, Error: %.9e",
