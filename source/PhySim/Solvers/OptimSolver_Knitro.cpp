...
int OptimSolver_Knitro::FunctionCallback(const int evalRequestCode,
                                         const int n,
                                         const int m,
                                         const int nnzJ,
                                         const int nnzH,
...
SolveResult OptimSolver_Knitro::SolveFull() {
  ...
  if (KTR_init_problem(kc, N, KTR_OBJGOAL_MINIMIZE, KTR_OBJTYPE_GENERAL,
                       vlb.data(), vub.data(),  // Objective
                       M, NULL, NULL, NULL, nnzJ, vrowsJ.data(),
                       vcolsJ.data(),  // Constraints
                       nnzH, vrowsH.data(), vcolsH.data(),
                       v0.data(),  // HessianFull
                       NULL) != 0) {
    logTrace(Verbosity::V1_Default,
  ...
  if (KTR_solve(kc, vx.data(), vl.data(), 0, &objective, NULL, NULL, NULL, NULL,
                NULL, m_pProblem) != 0) {
