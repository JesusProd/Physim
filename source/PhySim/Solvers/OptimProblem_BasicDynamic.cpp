...
void OptimProblem_BasicDynamic::Init(ISimulable* pM, Real dt) {
  ...
  m_pM->GetDOFVector(this->m_vx0, Tag_Position_X);
  m_pM->GetDOFVector(this->m_vv0, Tag_Velocity);
...
bool OptimProblem_BasicDynamic::GetEnergy(Real& e) {
  ...
  VectorXd vx;
  VectorXd vv;
  this->m_pM->GetDOFVector(vx, Tag_Position_X);
  this->m_pM->GetDOFVector(vv, Tag_Velocity);
...
bool OptimProblem_BasicDynamic::OnSolveStart(const OptimState& state) {
  ...
  m_pM->SetDOFVector(this->m_vx0 + this->m_dt * this->m_vv0, Tag_Position_X);
...
bool OptimProblem_BasicDynamic::PosPerformStep(const OptimState& state) {
  ...
  m_pM->GetDOFVector(vv, Tag_Position_X);
  vv -= this->m_vx0;
  vv /= this->m_dt;
  m_pM->SetDOFVector(vv, Tag_Velocity);
