...
namespace PhySim
{
	...
	LinearSolver_CudaSC::LinearSolver_CudaSC(const MatrixSd& mA, const LinearSolverOptions& options) : LinearSolver(mA, options)
	{
		devi_rows_coo = NULL;
		devi_cols_csr = NULL;
		devi_rows_csr = NULL;
	...
	void LinearSolver_CudaSC::Init(const MatrixSd& mA, const LinearSolverOptions& options)
	{
		...
		transformEigenTripletsToCUDA(vAFull, host_rows_coo, host_cols_csr, host_vals_csr);
		...
		cudaMemcpy(devi_rows_coo, host_rows_coo.data(), host_rows_coo.size() * sizeof(int), cudaMemcpyHostToDevice);
		cudaMemcpy(devi_cols_csr, host_cols_csr.data(), host_cols_csr.size() * sizeof(int), cudaMemcpyHostToDevice);
		cusparse_status = cusparseXcoo2csr(cusparse_handle, devi_rows_coo, numNZ, numRows, devi_rows_csr, CUSPARSE_INDEX_BASE_ZERO);
	}

	void LinearSolver_CudaSC::Free()
	{
		if (devi_b != NULL) cudaFree(devi_b);
		if (devi_x != NULL) cudaFree(devi_x);
		if (devi_t != NULL) cudaFree(devi_t);
		if (devi_rows_coo != NULL) cudaFree(devi_rows_coo);
		if (devi_rows_csr != NULL) cudaFree(devi_rows_csr);
		if (devi_cols_csr != NULL) cudaFree(devi_cols_csr);
		if (devi_vals_csr != NULL) cudaFree(devi_vals_csr);
		if (cusparse_handle != 0) cusparseDestroy(cusparse_handle);
	}

	SolveResult LinearSolver_CudaSC::SolveInternal(MatrixSd& mA, const VectorXd& vb, VectorXd& vx)
	{
		MatrixSd mAFull = mA.selfadjointView<Lower>();
		...
		cudaMemcpy(devi_b, host_b.data(), host_b.size() * sizeof(float), cudaMemcpyHostToDevice);
		...
		for (i = 0; i < this->m_options.regIters; ++i)
		{
			...
			cudaMemcpy(devi_vals_csr, host_vals_csr.data(), host_vals_csr.size() * sizeof(float), cudaMemcpyHostToDevice);

			// Solving

			int singular = 0;

			cudaDeviceSynchronize(); //logTrace(Verbosity::V1_Default, "\n[TRACE] CUDA Solver: synchronization %d", cudaDeviceSynchronize());

			cusparse_status = cusparseCreateMatDescr(&mA_descriptor);

			cusolver_status = cusolverSpScsrlsvchol(
				cusolver_handle, numRows, numNZ, mA_descriptor,
				devi_vals_csr, devi_rows_csr, devi_cols_csr,
				devi_b, 0, 1, devi_x,
				&singular);

			//logTrace(Verbosity::V1_Default, "\n[TRACE] CUDA Solver: system solved %d", cusolver_status);
			...
			float alpha = 1;
			float beta = 0;
			cusparseScsrmv(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE, numRows, numCols, numNZ, &alpha, mA_descriptor, devi_vals_csr, devi_rows_csr, devi_cols_csr, devi_x, &beta, devi_t);

			cudaDeviceSynchronize(); //logTrace(Verbosity::V1_Default, "\n[TRACE] CUDA Solver: synchronization: %d", cudaDeviceSynchronize());

			// Copy solution values device to host

			cudaMemcpy(host_x.data(), devi_x, numRows * sizeof(float), cudaMemcpyDeviceToHost);
			cudaMemcpy(host_t.data(), devi_t, numRows * sizeof(float), cudaMemcpyDeviceToHost);
			for (int j = 0; j < numRows; ++j)
			{
				vx(j) = host_x[j];
				vt(j) = host_t[j];
			...
			logTrace(Verbosity::V1_Default, "\n[SUCCESS] CUDA Solver: solved using Cholesky after %i regularization steps", i);
	...
	SolveResult LinearSolver_CudaSC::SolveInternal(MatrixSd& mA, const MatrixXd& mB, MatrixXd& mX)
	{
	...
	SolveResult LinearSolver_CudaSC::SolveInternal(MatrixSd& mA, const MatrixSd& mB, MatrixSd& mX)
	{
		throw new exception("Not implemented");
