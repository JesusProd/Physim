...
OptimSolver::OptimSolver(IOptimProblem* pProblem,
...
void OptimSolver::Init(IOptimProblem* pProblem,
                       const OptimSolverOptions& options) {
  ...
  IOUtils::logTrace(Verbosity::V1_Default, "\nInitial objective: %f",
                    this->m_state.m_obj);
  IOUtils::logTrace(Verbosity::V1_Default, "\nInitial optimality: %f",
                    this->m_state.m_opt);
  IOUtils::logTrace(Verbosity::V1_Default, "\nInitial feasibility: %f",
                    this->m_state.m_fea);
  IOUtils::logTrace(Verbosity::V1_Default, "\n--");
}
...
const OptimState& OptimSolver::SolveFull() {
  ...
  IOUtils::logTrace(Verbosity::V1_Default,
                    ...
                    "-----------------------------------------");
  IOUtils::logTrace(Verbosity::V1_Default,
                    "\nStarting problem %s SOLVE. Optimality: %f. Max. Steps: "
                    "%d. Max. Error: %f",
                    this->m_pProblem->GetName().c_str(), m_state.m_opt,
                    this->m_options.numMaxIters, this->m_options.tolMaxError);
  IOUtils::logTrace(Verbosity::V1_Default,
                    "\n--------------------------------------------------------"
                    "-----------------------------------------");
  IOUtils::logTrace(Verbosity::V1_Default,
  ...
  IOUtils::logTrace(
      ...
      "\nFinishing problem. Steps: %d, Optimality: %f. Status: %s", itCount,
      m_state.m_opt, status.c_str());
  IOUtils::logTrace(Verbosity::V1_Default,
                    "\n--------------------------------------------------------"
                    "-----------------------------------------");
  IOUtils::logTrace(Verbosity::V1_Default,
...
bool OptimSolver::LineSearch(const VectorXd& dxTry,
                             VectorXd& dxFin,
                             Real& objNew,
                             AVectorXd& vgNew,
                             int& numBis) {
  ...
  if (improved) {
    IOUtils::logTrace(Verbosity::V1_Default,
                      "\n[SUCCESS] Improved. Objective: %.9e. Optimality: "
                      "%.9e. Bis: %i. Step: %.9e\n--",
                      objNew, vgNew.norm(), numBis, dxFin.norm());
  } else {
    IOUtils::logTrace(Verbosity::V1_Default,
                      "\n[FAILURE] Non-Descent. Objective: %.9e. Optimality: "
                      "%.9e. Bis: %i. Step: %.9e\n--",
                      objNew, vgNew.norm(), numBis, dxFin.norm());
  }
...
void OptimSolver::UpdateStepResult(const VectorXd& dx,
                                   const Real& objNew,
...
bool OptimSolver::ComputeStep_BFGSDirect(VectorXd& dx, int trial) {
  ...
  this->m_timerLinearSolve.Start();
  this->m_pLSolver->Solve(this->m_bfgs.m_mBFGS, -(1.0) * this->m_state.m_vg,
                          dx);
  this->m_timerLinearSolve.StopStoreLog();
...
bool OptimSolver::UpdateBFGS_Inverse(const VectorXd& vxNew,
                                     const VectorXd& vgNew) {
  if (!this->m_bfgs.m_isInit)
    return this->InitialBFGS(vxNew, vgNew);
...
bool OptimSolver::UpdateBFGS_Direct(const VectorXd& vxNew,
                                    const VectorXd& vgNew) {
  if (!this->m_bfgs.m_isInit)
    return this->InitialBFGS(vxNew, vgNew);
  ...
  {
    IOUtils::logTrace(Verbosity::V1_Default,
                      "\n[SUCCESS] Updating BFGS matrix with discriminant %f",
                      D);

    Real rho = 1.0 / D;  // For too small discriminant, the update is invalid

    // Update the HessianFull matrix (direct one) (Numerical Optimization,
    // Nocedal, Wright, page 25) Hk+1 = (I - rho * m_sk * m_yk^T) * Hk * (I -
    // rho * m_yk * m_sk^T) + rho * m_yk * m_yk^T
    ...
    for (int i = 0; i < N; i++) {
      for (int j = 0; j <= i; j++) {
        if (i == j) {
          m_bfgs.m_mBFGSMod_A(i, i) =
              1.0 - rho * m_bfgs.m_vy[i] * m_bfgs.m_vs[i];
        } else {
    ...
    W = (m_bfgs.m_mBFGSMod_A * this->m_bfgs.m_mBFGS.toDense() *
             m_bfgs.m_mBFGSMod_A.transpose() +
         m_bfgs.m_mBFGSMod_B)
            .sparseView();
  } else {
    IOUtils::logTrace(Verbosity::V1_Default,
...
bool OptimSolver::LineSearch_Simple(const VectorXd& dxIn,
                                    VectorXd& dxOut,
                                    Real& objNew,
                                    AVectorXd& vgNew,
                                    int& numBis) {
  ...
  do {
    ...
    IOUtils::logTrace(Verbosity::V4_DeepShit,
                      "\n[TRACE] Trying LS iteration %d, step size: %.6e",
                      numBis, dxOut.norm());
  ...
  } while (dxOut.norm() > this->m_options.tolMinStep &&
           numBis <= this->m_options.lsearch_numTry);

  return improved;
}

bool OptimSolver::LineSearch_Armijo(const VectorXd& dxIn,
                                    VectorXd& dxOut,
                                    Real& objNew,
                                    AVectorXd& vgNew,
                                    int& numBis) {
  Real factor = 1;
  Real t = this->m_options.lSearch_wolfe1 *
           dxIn.normalized().dot(this->m_state.m_vg);
...
bool OptimSolver::LineSearch_WolfeWeak(const VectorXd& dxIn,
                                       VectorXd& dxOut,
                                       Real& objNew,
                                       AVectorXd& vgNew,
                                       int& numBis) {
...
bool OptimSolver::LineSearch_WolfeStrong(const VectorXd& dxIn,
                                         VectorXd& dxOut,
                                         Real& objNew,
                                         AVectorXd& vgNew,
                                         int& numBis) {
