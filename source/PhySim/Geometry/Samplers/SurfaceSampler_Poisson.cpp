...
struct WeightComparer {
 public:
  WeightComparer(const vector<double>& vWeights) : m_vWeights(vWeights) {}
...
SurfaceSampler_Poisson::SurfaceSampler_Poisson()
    : m_ratio(5), m_alpha(8.0), m_beta(0.65), m_gamma(1.5) {}
...
void SurfaceSampler_Poisson::Parameters(size_t ratio,
                                        double alpha,
                                        double beta,
                                        double gamma) {
  m_ratio = ratio;
...
void SurfaceSampler_Poisson::Sample(size_t M,
                                    vector<SurfaceSample>& vOutput) const {
...
void SurfaceSampler_Poisson::Elimination(size_t M,
                                         vector<SurfaceSample>& vInput,
                                         vector<SurfaceSample>& vOutput) const {
  ...
  //// the neighbors and pair-wise weights for each input point. Use a KD-Tree
  /// spatial / partition structure to accelerate this process.
  // vector<double> vWeights(N, 0.0);
  // vector<unordered_set<size_t>> vNeigh(N);
  // unordered_map<tuple<size_t, size_t>, double, Utils::XorHasher>
  // mNeighWeights;
  //
  ...
  //// Build a priority queue for input samples using the computed weights.
  /// Unfortunately, / using STL's priority_queue is inefficient when updating
  /// the weights. Instead, the set / data structure allows faster insertion and
  /// removal of objects, so use this structure / to mimick the priority queue
  /// behavior. /
  ...
  //// After the algorithm is complete, the remaining samples in the heap
  /// approximates / a Poisson-disk distribution. Copy the data to the output
  /// vector.
