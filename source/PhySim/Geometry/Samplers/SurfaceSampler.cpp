...
SurfaceSamplingStatistics::SurfaceSamplingStatistics(
...
void SurfaceSamplingStatistics::Setup(const vector<SurfaceSample>& vSamples) {
  ...
  // for (int i = 0; i < N; ++i)
  //{
  //    tree.FindKNN(m_mP.row(i).transpose(), res, 2); // 2 neighbors, as the
  //    first one will be the point itself. m_vD[i] = sqrt(res[1].second);
  //}
}
