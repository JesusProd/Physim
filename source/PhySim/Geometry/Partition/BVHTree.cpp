...
BVHTree::BVHTree() : m_pObject(nullptr), m_numElements(0) {}

BVHTree::BVHTree(Object* pObject, const Options& options)
    : m_pObject(pObject), m_options(options) {
  Rebuild();
}
...
void BVHTree::Refit() {
  assert(m_pObject != nullptr && "The given object is null.");
  assert(m_pObject->NumElements() == m_numElements &&
         "The number of elements in the object mismatchs.");
  Refit(0);
}

void BVHTree::Query(const AlignedBox3d& boundingBox,
...
void BVHTree::Intersect(IntersectionResults& entries, Real distance) const {
  ...
  Intersect(*this, root.LeftChildId, root.RightChildId, entries,
            distance * distance);
}

void BVHTree::Intersect(const BVHTree& other,
                        IntersectionResults& entries,
                        Real distance) const {
...
size_t BVHTree::Build(ptrdiff_t parentId,
                      bool isLeftChild,
  ...
  if (numElements <= m_options.MaxElementsPerLeaf ||
      depth >= m_options.MaxDepth) {
    Node leaf;
  ...
  if (parentId != -1) {
    if (isLeftChild == true)
      m_vNodes[parentId].LeftChildId = nodeId;
    else
      m_vNodes[parentId].RightChildId = nodeId;
  }
...
void BVHTree::Query(ptrdiff_t nodeId,
                    const AlignedBox3d& boundingBox,
                    QueryResults& entries,
                    Real sqrDistance) const {
  ...
  if (node.BBox.squaredExteriorDistance(boundingBox) <= sqrDistance) {
    if (node.IsLeafNode == true) {
      ...
      entries.insert(entries.end(), node.ElementIds.begin(),
                     node.ElementIds.end());
    } else {
  ...
  if (node.BBox.squaredExteriorDistance(otherNode.BBox) <= sqrDistance) {
    if (node.IsLeafNode == true && otherNode.IsLeafNode == true) {
      size_t totalNumElements =
          node.ElementIds.size() * otherNode.ElementIds.size();
      ...
      Intersect(other, node.LeftChildId, otherNode.LeftChildId, entries,
                sqrDistance);
      Intersect(other, node.LeftChildId, otherNode.RightChildId, entries,
                sqrDistance);
      Intersect(other, node.RightChildId, otherNode.LeftChildId, entries,
                sqrDistance);
      Intersect(other, node.RightChildId, otherNode.RightChildId, entries,
                sqrDistance);
    }
  }
}
