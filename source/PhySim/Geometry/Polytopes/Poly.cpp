...
namespace PhySim
{
	...
	void Poly::MassProperties(Tag s, Real rho, Real& mass, Vector3d& vcom, Matrix3d& mI) const
	{
		throw exception("Not implemented");
	}
	...
	{
		PtrS<IShapeFunction::IDeformationData> pData = this->m_pSFunction->CreateDeformationData();
		pData->SetPoly(this);
		pData->SetTag0(f);
		pData->SetTagX(t);
		vector<VectorXd> vvp;
	...
	VectorXd Poly::InterpolateDeformation(const VectorXd& vpoint, Tag trait0, Tag traitX) const
	{
		MatrixXd mP(1, vpoint.size());
		MatrixXd mI(1, vpoint.size());
