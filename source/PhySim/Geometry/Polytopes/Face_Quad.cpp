...
namespace PhySim
{
	...
	Face_Quad::Face_Quad(int id, const vector<Node*>& m_vnodes) : Face(id, m_vnodes)
	{
		// Nothing to do here...
	}
	...
	void Face_Quad::TransformNat2Iso(VectorXd& vb, MatrixXd& mA, Tag s) const
	{
		...
		Vector3d e1 = this->m_vnodes[3]->Traits().Vector3d(s) - this->m_vnodes[0]->Traits().Vector3d(s);
		mNB.col(0) = e0;
		mNB.col(1) = e1;
	...
	{
		mA.resize(3, 2);
		mA.col(0) = this->m_vnodes[1]->Traits().Vector3d(s) - this->m_vnodes[0]->Traits().Vector3d(s);
		mA.col(1) = this->m_vnodes[3]->Traits().Vector3d(s) - this->m_vnodes[0]->Traits().Vector3d(s);
		vb = this->Centroid(s);
	}
