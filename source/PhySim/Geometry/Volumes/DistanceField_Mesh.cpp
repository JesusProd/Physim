...
void DistanceField_Mesh::Setup(const MatrixXd& mPoints,
                               const MatrixXi& mTriangles,
                               int LargestAxisSize,
                               int Padding) {
  m_mPoints = mPoints;
  m_mFaces = mTriangles;
  ...
  // Up to this point, we have safely determined the closest primitive and its
  // distance for each voxel in the volume. All that remains is to determine
  // their sign so we can discern between outside or inside of the volume.
  DetermineSigns();
}
...
void DistanceField_Mesh::Compare(const Vector3d& vPoint,
                                 const Vector3i& vIndex,
                                 const Vector3i& vDeltaIndex) {
...
void DistanceField_Mesh::Rasterize() {
  ...
  for (size_t i = 0; i < M; ++i) {
    ...
    Vector3i vFaceMinIndex =
        IndexOf(vFaceMinAABB) - Vector3i::Constant(narrowBand);
    Vector3i vFaceMaxIndex =
        IndexOf(vFaceMaxAABB) + Vector3i::Constant(narrowBand);
...
void DistanceField_Mesh::DetermineSigns() {
  ...
  // With all this information, we are now ready to compute the signed distance
  // for all the points within the volume.
  for (int z = 0; z < vShape[2]; ++z)
    for (int y = 0; y < vShape[1]; ++y)
      for (int x = 0; x < vShape[0]; ++x) {
        ...
        m_sfDistances[{x, y, z}] *=
            GeometryUtils::determineTriangleDistanceSign(
                vP, vA, vB, vC, vNA, vNB, vNC, vNAB, vNBC, vNCA);
      }
  ...
  for (int z = z0; z != z1; z += vDeltaIndex[2])
    for (int y = y0; y != y1; y += vDeltaIndex[1])
      for (int x = x0; x != x1; x += vDeltaIndex[0]) {
        ...
        Compare(vP, vI,
                Vector3i(-vDeltaIndex[0], -vDeltaIndex[1], -vDeltaIndex[2]));
      }
}
