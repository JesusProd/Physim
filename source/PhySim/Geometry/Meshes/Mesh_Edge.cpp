...
namespace PhySim
{
	...
	Mesh_Edge::Mesh_Edge(const MatrixXd& mV, const MatrixXi& mE, Discretization D, const vector<Tag>& vnTraits) : Mesh(mV, mE, D, vnTraits)
	{
		this->UpdateMetadata();
	}

	void Mesh_Edge::Init(const MatrixXd& mV, const MatrixXi& mE, Discretization D, const vector<Tag>& vnTraits)
	{
	...
	void Mesh_Edge::Merge(PtrS<Mesh_Edge>& pMesh, const vector<PtrS<Mesh_Edge>>& vmeshes, Tag trait, Real tol)
	{
	...
	void Mesh_Edge::UpdateMetadata()
	{
		...
		for (int i = 0; i < this->m_vedges.size(); ++i)
		{
			...
			for (int j = 0; j < m_vnodeAdjaEdges[pe->GetTail()->ID()].size(); ++j)
			{
				...
				if (po != pe) vedgeAdjaEdges[pe->ID()].push_back(po);
			}
		...
		for (int i = 0; i < this->m_vedges.size(); ++i)
		{
			...
			for (int i = 0; i < m_vnodeAdjaEdges[pe->GetTail()->ID()].size(); ++i)
				...
				{
					if (m_vnodeAdjaEdges[pe->GetTail()->ID()][i]->GetHead() == pe->GetTail())
					{
						this->m_vedgePrevEdges[pe->ID()].push_back(m_vnodeAdjaEdges[pe->GetTail()->ID()][i]);
					}
				}
			...
			for (int i = 0; i < m_vnodeAdjaEdges[pe->GetHead()->ID()].size(); ++i)
				...
				{
					if (m_vnodeAdjaEdges[pe->GetHead()->ID()][i]->GetTail() == pe->GetHead())
					{
						this->m_vedgeNextEdges[pe->ID()].push_back(m_vnodeAdjaEdges[pe->GetHead()->ID()][i]);
					}
				}
		}
		...
		for (int i = 0; i < m_vnodes.size(); ++i)
		{
			...
			for (int i = 0; i < numE; ++i)
			{
				...
				else pc->m_vedges[i]->Traits().AddTrait(Tag::Tag_Sign_0, int(-1));
			}
