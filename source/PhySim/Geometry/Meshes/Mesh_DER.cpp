...
namespace PhySim
{
	...
	Mesh_DER::Mesh_DER(const MatrixXd& mV, const MatrixXi& mE, const vector<Frame3d>& vF, const vector<Vector2d>& vr)
	{
	...
	void Mesh_DER::Init(const MatrixXd& mV, const MatrixXi& mE, const vector<Frame3d>& vF, const vector<Vector2d>& vr)
	{
		...
		{
			m_vconns[i]->m_center->Traits().AddTrait<Matrix3d>(Tag::Tag_Rotat_X, Matrix3d::Identity());
			m_vconns[i]->m_center->Traits().AddTrait<Vector3d>(Tag::Tag_Euler_X, Vector3d(0.0, 0.0, 0.0));
			m_vconns[i]->m_center->Traits().AddTrait<Vector3d>(Tag::Tag_Euler_V, Vector3d(0.0, 0.0, 0.0));

			for (int j = 0; j < (int) m_vconns[i]->m_vedges.size(); ++j)
			{
				m_vconns[i]->m_vedges[j]->Traits().AddTrait<double>(Tag::Tag_Twist_X, 0.0);
			}
		}
	...
	void Mesh_DER::ComputeMatFrameArrows(MatrixXd& m0, MatrixXd& mN, MatrixXd& mB, Tag s)
	{
		vector<Frame3d> vF;
	...
	Real& Mesh_DER::MatAngle(int i, Tag s)
	{
		...
		throw PhySim::exception("Invalid Tag, expected: PosX, Pos0, Angle_X, Angle_0");
	}
	...
	Frame3d& Mesh_DER::RefFrame(int i, Tag s)
	{
		...
		throw PhySim::exception("Invalid Tag, expected: PosX, Pos0, Frame_X, Frame_0");
	}
	...
	void Mesh_DER::AdaptReferenceFrame(int i, Tag s)
	{
		...
		if (s == Tag_Position_X || s == Tag_Frame_X)
		{
			pEdge->Traits().Frame3d(Tag::Tag_Frame_X) = GeometryUtils::parallelTransport(pEdge->Traits().Frame3d(Tag::Tag_Frame_X), pEdge->Tangent(Tag_Position_X));
			return;
		...
		if (s == Tag_Position_0 || s == Tag_Frame_0)
		{
			pEdge->Traits().Frame3d(Tag::Tag_Frame_0) = GeometryUtils::parallelTransport(pEdge->Traits().Frame3d(Tag::Tag_Frame_0), pEdge->Tangent(Tag_Position_0));
			return;
		}
	...
	void Mesh_DER::ComputeReferenceTwist(int i)
	{
		...
		rtX += computeReferenceTwistChange(Fprev.tan.data(), Fprev.nor.data(), Fnext.tan.data(), Fnext.nor.data(), rtX);
	}
	...
	void Mesh_DER::UpdateConnectionRotation(int i)
	{
		...
		GeometryUtils::eulerRotation(veuler.data(), mstart.col(0).data(), mrotat.col(0).data());
                GeometryUtils::eulerRotation(
                    veuler.data(), mstart.col(1).data(), mrotat.col(1).data());
                GeometryUtils::eulerRotation(
                    veuler.data(), mstart.col(2).data(), mrotat.col(2).data());
		mstart = mrotat;
		veuler.setZero();
		...
		for (int i = 0; i < (int) pConn->m_vedges.size(); ++i)
		{
			...
			rtX += computeReferenceTwistChange(FR0.tan.data(), FR0.nor.data(), FX.tan.data(), FX.nor.data(), rtX);
		}
	}
