...
void Mesh_DER::Init(const vector<PtrS<Rod>>& vrods, Tag trait, Real tol) {
  ...
  if (pTempMesh->HasNodeTraits(Tag::Position_0)) {
    MatrixXd mV;
    pTempMesh->GetNodesTrait(mV, Tag::Position_0);
    this->SetNodesTrait(mV, Tag::Position_0);
  }

  if (pTempMesh->HasNodeTraits(Tag::Position_X)) {
    MatrixXd mV;
    pTempMesh->GetNodesTrait(mV, Tag::Position_X);
    this->SetNodesTrait(mV, Tag::Position_X);
  }

  if (pTempMesh->HasNodeTraits(Tag::Velocity)) {
    MatrixXd mV;
    pTempMesh->GetNodesTrait(mV, Tag::Velocity);
    this->SetNodesTrait(mV, Tag::Velocity);
  }

  if (pTempMesh->HasElemTraits(Tag::Frame_X)) {
    vector<Frame3d> vF;
    pTempMesh->GetElemsTrait(vF, Tag::Frame_X);
    this->SetElemsTrait(vF, Tag::Frame_X);
  }

  if (pTempMesh->HasElemTraits(Tag::Frame_0)) {
    vector<Frame3d> vF;
    pTempMesh->GetElemsTrait(vF, Tag::Frame_0);
    this->SetElemsTrait(vF, Tag::Frame_0);
  }
}
...
void Mesh_DER::Init(const MatrixXd& mV,
  ...
  vnodeTraits.push_back(Tag::Position_0);
  vnodeTraits.push_back(Tag::Position_X);
  vnodeTraits.push_back(Tag::Velocity);

  vector<Tag> vframeTraits;
  vframeTraits.push_back(Tag::Frame_0);
  vframeTraits.push_back(Tag::Frame_X);
  ...
  for (int i = 0; i < (int)m_velems.size(); ++i) {
    m_velems[i]->Traits().AddTrait<Real>(Tag::Angle_0, 0.0);
    m_velems[i]->Traits().AddTrait<Real>(Tag::Angle_X, 0.0);
    m_velems[i]->Traits().AddTrait<Real>(Tag::Angle_V, 0.0);
  }

  // Add reference twist at nodes

  for (int i = 0; i < (int)m_vnodes.size(); ++i) {
    m_vnodes[i]->Traits().AddTrait<Real>(Tag::Twist_X, 0.0);
  }
  ...
  if (vr.size() == 1) {
    for (int i = 0; i < (int)m_velems.size(); ++i) {
      m_velems[i]->Traits().AddTrait<Real>(Tag::Size_0, vr[0].x());
      m_velems[i]->Traits().AddTrait<Real>(Tag::Size_1, vr[0].y());
    }
  } else if (vr.size() == m_velems.size()) {
    for (int i = 0; i < (int)m_velems.size(); ++i) {
      m_velems[i]->Traits().AddTrait<Real>(Tag::Size_0, vr[i].x());
      m_velems[i]->Traits().AddTrait<Real>(Tag::Size_1, vr[i].y());
    }
  } else {
    for (int i = 0; i < (int)m_velems.size(); ++i) {
      m_velems[i]->Traits().AddTrait<Real>(Tag::Size_0, 0.01);
      m_velems[i]->Traits().AddTrait<Real>(Tag::Size_1, 0.01);
    }
  }
  ...
  for (int i = 0; i < (int)m_vconns.size(); ++i) {
    m_vconns[i]->m_center->Traits().AddTrait<Matrix3d>(Tag::Rotat_X,
                                                       Matrix3d::Identity());
    m_vconns[i]->m_center->Traits().AddTrait<Vector3d>(Tag::Euler_X,
                                                       Vector3d(0.0, 0.0, 0.0));
    m_vconns[i]->m_center->Traits().AddTrait<Vector3d>(Tag::Euler_V,
                                                       Vector3d(0.0, 0.0, 0.0));

    for (int j = 0; j < (int)m_vconns[i]->m_vedges.size(); ++j) {
      m_vconns[i]->m_vedges[j]->Traits().AddTrait<double>(Tag::Twist_X, 0.0);
    }
  }

  this->SetRefFrames_TwistFreeRods(Tag::Frame_0);
  this->SetRefFrames_TwistFreeRods(Tag::Frame_X);
...
Frame3d Mesh_DER::ComputeMatFrame(int i, Tag s) {
  ...
  if (s == Tag::Position_0 || s == Tag::Frame_0 || s == Tag::Angle_0) {
    refF = this->RefFrame(i, Tag::Frame_0);
    twist = this->MatAngle(i, Tag::Angle_0);
  }

  if (s == Tag::Position_X || s == Tag::Frame_X || s == Tag::Angle_X) {
    refF = this->RefFrame(i, Tag::Frame_X);
    twist = this->MatAngle(i, Tag::Angle_X);
  }
...
void Mesh_DER::ComputeMatFrameArrows(MatrixXd& m0,
  ...
  for (int i = 0; i < this->NumEdges(); ++i) {
    ...
    if (s == Tag::Position_0 || s == Tag::Frame_0 || s == Tag::Angle_0)
      vedgeCen = this->m_vedges[i]->Centroid(Tag::Position_0);

    if (s == Tag::Position_X || s == Tag::Frame_X || s == Tag::Angle_X)
      vedgeCen = this->m_vedges[i]->Centroid(Tag::Position_X);
...
Real& Mesh_DER::MatAngle(int i, Tag s) {
  if (s == Tag::Position_X || s == Tag::Angle_X)
    return this->GetEdge(i)->Traits().Double(Tag::Angle_X);

  if (s == Tag::Position_0 || s == Tag::Angle_0)
    return this->GetEdge(i)->Traits().Double(Tag::Angle_0);
...
Frame3d& Mesh_DER::RefFrame(int i, Tag s) {
  if (s == Tag::Position_X || s == Tag::Frame_X)
    return this->GetEdge(i)->Traits().Frame3d(Tag::Frame_X);

  if (s == Tag::Position_0 || s == Tag::Frame_0)
    return this->GetEdge(i)->Traits().Frame3d(Tag::Frame_0);
...
Real& Mesh_DER::RefTwist(int i) {
  return this->Nodes()[i]->Traits().Double(Tag::Twist_X);
}

Vector3d& Mesh_DER::RotEuler(int i) {
  return this->Connections()[i]->m_center->Traits().Vector3d(Tag::Euler_X);
}

Matrix3d& Mesh_DER::RotMatrix(int i) {
  return this->Connections()[i]->m_center->Traits().Matrix3d(Tag::Rotat_X);
}
...
void Mesh_DER::AdaptReferenceFrame(int i, Tag s) {
  ...
  if (s == Tag::Position_X || s == Tag::Frame_X) {
    pEdge->Traits().Frame3d(Tag::Frame_X) = GeometryUtils::parallelTransport(
        pEdge->Traits().Frame3d(Tag::Frame_X), pEdge->Tangent(Tag::Position_X));
    return;
  }

  if (s == Tag::Position_0 || s == Tag::Frame_0) {
    pEdge->Traits().Frame3d(Tag::Frame_0) = GeometryUtils::parallelTransport(
        pEdge->Traits().Frame3d(Tag::Frame_0), pEdge->Tangent(Tag::Position_0));
    return;
  }
...
void Mesh_DER::ComputeReferenceTwist(int i) {
  ...
  Real& rtX = pNode->Traits().Double(Tag::Twist_X);
  Frame3d& Fprev = vprevEdges[0]->Traits().Frame3d(Tag::Frame_X);
  Frame3d& Fnext = vnextEdges[0]->Traits().Frame3d(Tag::Frame_X);
...
void Mesh_DER::UpdateConnectionRotation(int i) {
  ...
  Matrix3d& mstart = pConn->m_center->Traits().Matrix3d(Tag::Rotat_X);
  Vector3d& veuler = pConn->m_center->Traits().Vector3d(Tag::Euler_X);
  ...
  for (int i = 0; i < (int)pConn->m_vedges.size(); ++i) {
    Real& rtX = pConn->m_vedges[i]->Traits().Double(Tag::Twist_X);
    Frame3d& F0 = pConn->m_vedges[i]->Traits().Frame3d(Tag::Frame_0);
    Frame3d& FX = pConn->m_vedges[i]->Traits().Frame3d(Tag::Frame_X);
    Frame3d FR0;
