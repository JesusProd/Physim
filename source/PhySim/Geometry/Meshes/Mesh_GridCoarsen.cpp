...
namespace PhySim
{
	...
	Mesh_GridCoarsen::Mesh_GridCoarsen(const Mesh_GridCoarsen& toCopy) : Mesh_Grid(toCopy)
	{
	...
	Mesh_GridCoarsen::Mesh_GridCoarsen(const Vector3d& vorigin, const Vector3d& vsize, const Vector3i& vfineDims, const Vector3i& vcoarDims, const iVector& vfineOccu, Discretization D, const vector<Tag>& vnTraits) : Mesh_Grid()
	{
		this->Init(vorigin, vsize, vfineDims, vcoarDims, vfineOccu, D, vnTraits);
	}

	void Mesh_GridCoarsen::Init(const Vector3d& vorigin, const Vector3d& vsize, const Vector3i& vfineDims, const Vector3i& vcoarDims, const iVector& vfineOccu, Discretization D, const vector<Tag>& vnTraits)
	{
		// Initialize fine mesh

		this->m_pFineMesh.reset(new Mesh_Grid(vorigin, vsize, vfineDims, vfineOccu, D, vnTraits));
				...
				for (int k = 0; k < vcoarDims.z(); ++k)
				{
					...
					for (int ii = minI; ii <= maxI && !found; ++ii)
						for (int jj = minJ; jj <= maxJ && !found; ++jj)
							...
							{
								if (this->m_pFineMesh->MapFull2Active_Elems()[this->m_pFineMesh->TransformRegularCoordToIndex_Elem(Vector3i(ii, jj, kk))] != -1)
									found = true;
							}

					if (found)
						vcoarOccu.push_back(TransformRegularCoordToIndex(vcoarDims.x(), vcoarDims.y(), vcoarDims.z(), Vector3i(i, j, k)));
				}
	...
	Mesh_GridCoarsen* Mesh_GridCoarsen::CreateSubCuboid(double sizeX, double sizeY, double sizeZ,
																Vector2d ranX, Vector2d ranY, Vector2d ranZ,
																int coarseElements, int coarseningLevel,
																vector<Tag> vnTraits)
	{
		...
		return new Mesh_GridCoarsen(-vsize*0.5,
									vsize,
									Vector3i(fineDimX, fineDimY, fineDimZ),
									Vector3i(coarDimX, coarDimY, coarDimZ),
									voccupancy, Discretization::Discretization_Hex8, vnTraits);
	}

	Mesh_GridCoarsen* Mesh_GridCoarsen::CreateFullCuboid(double sizeX, double sizeY, double sizeZ,
														 int coarseElements, int coarseningLevel,
													     vector<Tag> vnTraits)
	{
		int coarDimX = coarseElements;
	...
	Mesh_GridCoarsen* Mesh_GridCoarsen::CreateSphere(const Vector3d& center, double radius,
													 int coarseElements, int coarseningLevel,
													 vector<Tag> vnTraits)
	{
		int coarDimX = coarseElements;
	...
	void Mesh_GridCoarsen::UpdateMetadata()
	{
		...
		for (int i = 0; i < this->m_pFineMesh->NumNodes(); ++i)
			this->m_pFineMesh->Nodes()[i]->Traits().AddTrait<FineNodeMeta>(Tag::Tag_FineMeta, FineNodeMeta());
		for (int i = 0; i < this->m_pFineMesh->NumElems(); ++i)
			this->m_pFineMesh->Elems()[i]->Traits().AddTrait<FineElemMeta>(Tag::Tag_FineMeta, FineElemMeta());

		for (int i = 0; i < this->NumNodes(); ++i)
			this->Nodes()[i]->Traits().AddTrait<CoarNodeMeta>(Tag::Tag_CoarMeta, CoarNodeMeta());
		for (int i = 0; i < this->NumElems(); ++i)
			this->Elems()[i]->Traits().AddTrait<CoarElemMeta>(Tag::Tag_CoarMeta, CoarElemMeta());
	...
	void Mesh_GridCoarsen::InitializeCoarseElements_RegularGrid()
	{
		...
		for (int i = 0; i < this->m_pFineMesh->NumElems(); ++i)
		{
			Vector3i fineElemFullCoord = this->m_pFineMesh->TransformRegularIndexToCoord_Elem(this->m_pFineMesh->MapActive2Full_Elems()[i]);
			Vector3i coarElemFullCoord(
				fineElemFullCoord.x() / this->m_vcoarsening.x(),
				fineElemFullCoord.y() / this->m_vcoarsening.y(),
				fineElemFullCoord.z() / this->m_vcoarsening.z());
			int coarElemFullIndex = this->TransformRegularCoordToIndex_Elem(coarElemFullCoord);
	...
	{
		CoarElemMeta& coarElemMeta = this->Elems()[coarElemIdx]->Trait<CoarElemMeta>(Tag::Tag_CoarMeta);
		...
		for (int j = 0; j < (int)coarElemMeta.vfineElem.size(); ++j)
		{
			...
			GridElemMeta& gridElemMeta = pFineElem->Trait<GridElemMeta>(Tag::Tag_GridMeta);
			FineElemMeta& fineElemMeta = pFineElem->Trait<FineElemMeta>(Tag::Tag_FineMeta);
	...
	{
		GridElemMeta& gridElemMeta = this->Elems()[coarElemIdx]->Trait<GridElemMeta>(Tag::Tag_GridMeta);
		CoarElemMeta& coarElemMeta = this->Elems()[coarElemIdx]->Trait<CoarElemMeta>(Tag::Tag_CoarMeta);
		...
		{ // X-
			...
			{
				int neigElemIdx = coarElemMeta.vboundaryXm[i]->elem0->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem->ID();
		...
		{ // X+
			...
			{
				int neigElemIdx = coarElemMeta.vboundaryXp[i]->elem1->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem->ID();
				if (sneighbors.find(neigElemIdx) == sneighbors.end())
				{
		...
		{ // Y-
			...
			{
				int neigElemIdx = coarElemMeta.vboundaryYm[i]->elem0->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem->ID();
				if (sneighbors.find(neigElemIdx) == sneighbors.end())
				{
					pair<int, int> key(neigElemIdx, coarElemIdx);
					if (this->m_mdualEdges.find(key) == m_mdualEdges.end())
		...
		{ // Y+
			...
			{
				int neigElemIdx = coarElemMeta.vboundaryYp[i]->elem1->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem->ID();
				if (sneighbors.find(neigElemIdx) == sneighbors.end())
				{
					pair<int, int> key(coarElemIdx, neigElemIdx);
					if (this->m_mdualEdges.find(key) == m_mdualEdges.end())
		...
		{ // Z-
			...
			{
				int neigElemIdx = coarElemMeta.vboundaryZm[i]->elem0->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem->ID();
				if (sneighbors.find(neigElemIdx) == sneighbors.end())
				{
					pair<int, int> key(neigElemIdx, coarElemIdx);
					if (this->m_mdualEdges.find(key) == m_mdualEdges.end())
		...
		{ // Z+
			...
			{
				int neigElemIdx = coarElemMeta.vboundaryZp[i]->elem1->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem->ID();
				if (sneighbors.find(neigElemIdx) == sneighbors.end())
				{
					pair<int, int> key(coarElemIdx, neigElemIdx);
					if (this->m_mdualEdges.find(key) == m_mdualEdges.end())
	...
	void Mesh_GridCoarsen::RecomputeCoarseFineSubmesh(int coarElemIdx)
	{
		...
		for (int j = 0; j < (int)coarElemMeta.vfineElem.size(); ++j)
		{
			...
			for (int k = 0; k < pFineElem->NumNodes(); ++k)
			{
				...
				map<int, int>& mlocalFullIdx = pFineNode->Trait<FineNodeMeta>(Tag_FineMeta).mLocalFullIdx;
				map<int, int>& mlocalActiveIdx = pFineNode->Trait<FineNodeMeta>(Tag_FineMeta).mLocalActiveIdx;
			...
			Vector3i globalFullElemCoord = this->m_pFineMesh->TransformRegularIndexToCoord_Elem(globalFullElemIdx);
			Vector3i localFullElemCoord(globalFullElemCoord.x() % this->m_vcoarsening.x(),
				globalFullElemCoord.y() % this->m_vcoarsening.y(),
				globalFullElemCoord.z() % this->m_vcoarsening.z());
			int localFullElemIdx = this->TransformRegularCoordToIndex(this->m_vcoarsening.x(),
				this->m_vcoarsening.y(),
				this->m_vcoarsening.z(),
				localFullElemCoord);
		...
		for (int j = 0; j < (int)coarElemMeta.vfineElem.size(); ++j)
		{
			...
			for (int k = 0; k < pFineElem->NumNodes(); ++k)
			{
				...
				if (mlocalActiveIdx.find(coarElemIdx) == mlocalActiveIdx.end())
				{
					...
					Vector3i localFullCoord(
						(int)floor(alphaX*m_vcoarsening.x()),
						(int)floor(alphaY*m_vcoarsening.y()),
						(int)floor(alphaZ*m_vcoarsening.z()));
					int localFullIndex = this->TransformRegularCoordToIndex(m_vcoarsening.x() + 1,
																			m_vcoarsening.y() + 1,
																			m_vcoarsening.z() + 1,
																			localFullCoord);
		...
		coarElemMeta.vfineNode.insert(coarElemMeta.vfineNode.end(), sfineNode.begin(), sfineNode.end());
		...
		struct NodeLesser
		{
			...
			NodeLesser(int coarElemIdx)
			{
				this->m_coarElemIdx = coarElemIdx;
			}
		...
		std::sort(
			coarElemMeta.vfineNode.begin(),
			coarElemMeta.vfineNode.end(),
		...
		{
			int found = coarElemMeta.vfineNode[i]->Trait<FineNodeMeta>(Tag::Tag_FineMeta).mLocalActiveIdx[coarElemIdx];
			if (found != i)
			{
                          IOUtils::logTrace(
                              Verbosity::V1_Default,
                              "[ERROR] Expected local index %d, found %d", i,
                              found);
				assert(false);
			}
		}
		...
		for (auto it = coarElemMeta.vfineNode.begin(); it != coarElemMeta.vfineNode.end(); ++it)
		{
			const Vector3d vpos = (*it)->Traits().Vector3d(Tag::Tag_Position_0);
			Embedding embed = *Elems()[coarElemIdx]->ComputeEmbedding(vpos, Tag::Tag_Position_0);
			if (embed.Valid()) (*it)->Traits().AddTrait<Embedding>(Tag::Tag_Embedding_0, embed);
		}
	}

	void Mesh_GridCoarsen::GetSharedNodes(int coarElemIdx, int elemSide, vector<SharedNodes>& vshared)
	{
		CoarElemMeta& coarElemMeta = this->Elems()[coarElemIdx]->Trait<CoarElemMeta>(Tag::Tag_CoarMeta);
		...
		switch (elemSide)
		{
		case 0: vboundaryEdges = &coarElemMeta.vboundaryXm; break;
		case 1: vboundaryEdges = &coarElemMeta.vboundaryXp; break;
		case 2: vboundaryEdges = &coarElemMeta.vboundaryYm; break;
		case 3: vboundaryEdges = &coarElemMeta.vboundaryYp; break;
		case 4: vboundaryEdges = &coarElemMeta.vboundaryZm; break;
		case 5: vboundaryEdges = &coarElemMeta.vboundaryZp; break;
		}
		...
		for (int j = 0; j < (int) vboundaryEdges->size(); ++j)
		{
			...
			int otherCoarIdx = pOtherElem->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem->ID();
			...
			{
			case 0: // Negative X
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[0], pOtherElem->Nodes()[1]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[3], pOtherElem->Nodes()[2]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[7], pOtherElem->Nodes()[6]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[4], pOtherElem->Nodes()[5]));
				break;
			case 1: // Positive X
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[1], pOtherElem->Nodes()[0]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[2], pOtherElem->Nodes()[3]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[6], pOtherElem->Nodes()[7]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[5], pOtherElem->Nodes()[4]));
				break;
			case 2: // Negative Y
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[0], pOtherElem->Nodes()[3]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[4], pOtherElem->Nodes()[7]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[5], pOtherElem->Nodes()[6]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[1], pOtherElem->Nodes()[2]));
				break;
			case 3: // Positive Y
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[3], pOtherElem->Nodes()[0]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[7], pOtherElem->Nodes()[4]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[6], pOtherElem->Nodes()[5]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[2], pOtherElem->Nodes()[1]));
				break;
			case 4: // Negative Z
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[0], pOtherElem->Nodes()[4]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[1], pOtherElem->Nodes()[5]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[2], pOtherElem->Nodes()[6]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[3], pOtherElem->Nodes()[7]));
				break;
			case 5: // Positive Z
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[4], pOtherElem->Nodes()[0]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[5], pOtherElem->Nodes()[1]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[6], pOtherElem->Nodes()[2]));
				pairSet.insert(pair<Node*, Node*>(pThisElem->Nodes()[7], pOtherElem->Nodes()[3]));
				break;
			}
		}
		...
		for (auto it = mcollect.begin(); it != mcollect.end(); ++it)
		{
			...
			for (; itCurPairs != itEndPairs; ++itCurPairs)
			{
				...
				vshared.back().vnodeIdx0.push_back(itCurPairs->first->Trait<FineNodeMeta>(Tag::Tag_FineMeta).mLocalActiveIdx[coarElemIdx]);
				vshared.back().vnodeIdx1.push_back(itCurPairs->second->Trait<FineNodeMeta>(Tag::Tag_FineMeta).mLocalActiveIdx[it->first]);
				vshared.back().vFineNodes.push_back(itCurPairs->first);
			}
		}
	}

	Mesh_GridCoarsen::TopologyChanges Mesh_GridCoarsen::CutMesh_Rect(Real length, Real height, const Matrix3d& mR, const Vector3d& vt, Tag vs)
	{
	...
	Mesh_GridCoarsen::TopologyChanges Mesh_GridCoarsen::CutMesh_Disc(Real radius, const Matrix3d& mR, const Vector3d& vt, Tag vs)
	{
		// Compute potential cuts
	...
	Mesh_GridCoarsen::TopologyChanges Mesh_GridCoarsen::CutMesh(const list<PtrS<EdgeCut>>& lcutList)
	{
		...
		lfineAffected.insert(lfineAffected.begin(), 
							 sfineAffected.begin(), 
							 sfineAffected.end());
	...
	void Mesh_GridCoarsen::RecomputeCoarse(list<Poly*>& lfineAffected, Mesh_GridCoarsen::TopologyChanges& changes)
	{
		set<Poly*> lcoarAffected;
		for (auto itCur = lfineAffected.begin(); itCur != lfineAffected.end(); ++itCur)
			lcoarAffected.insert((*itCur)->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem);
		...
		for (auto itCur = lcoarAffected.begin(); itCur != lcoarAffected.end(); ++itCur)
		{
			Cell_Hexa* pOldCoarElem = (Cell_Hexa*)(*itCur);
			GridElemMeta& oldGridElemMeta = pOldCoarElem->Trait<GridElemMeta>(Tag::Tag_GridMeta);
			CoarElemMeta& oldCoarElemMeta = pOldCoarElem->Trait<CoarElemMeta>(Tag::Tag_CoarMeta);
			...
			for (int i = 0; i < (int)oldCoarElemMeta.vfineElem.size(); ++i)
				oldCoarElemMeta.vfineElem[i]->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem = NULL;
			...
			{
				if (oldCoarElemMeta.vfineElem[i]->Trait<FineElemMeta>(Tag::Tag_FineMeta).coarElem == NULL)
				{
	...
	Cell_Hexa* Mesh_GridCoarsen::CreateCoarseElement(const Cell_Hexa* pOldCoarElem, const list<Poly*>& lchildren)
	{
		...
		pNewCoarElem->Traits().AddTrait<GridElemMeta>(Tag::Tag_GridMeta, GridElemMeta());
		pNewCoarElem->Traits().AddTrait<CoarElemMeta>(Tag::Tag_CoarMeta, CoarElemMeta());
		GridElemMeta& newGridElemMeta = pNewCoarElem->Trait<GridElemMeta>(Tag::Tag_GridMeta);
		CoarElemMeta& newCoarElemMeta = pNewCoarElem->Trait<CoarElemMeta>(Tag::Tag_CoarMeta);
		...
		for (auto it = lchildren.begin(); it != lchildren.end(); ++it)
		{
			...
			sfineNodes.insert(pFineElem->Nodes().begin(),
				pFineElem->Nodes().end());
		...
		newCoarElemMeta.vfineNode.insert(newCoarElemMeta.vfineNode.end(), sfineNodes.begin(), sfineNodes.end());

		return pNewCoarElem;
	}

	void Mesh_GridCoarsen::ListConnectedOrphanElements(Poly* pRoot, list<Poly*>& lfine)
	{
		// To store selected
		...
		while (!lqueue.empty())
		{
			...
			{
				if (!gridElemMeta.vDualEdgesXm.empty()) lqueue.push_back(gridElemMeta.vDualEdgesXm[0]->elem0);
				if (!gridElemMeta.vDualEdgesXp.empty()) lqueue.push_back(gridElemMeta.vDualEdgesXp[0]->elem1);
				if (!gridElemMeta.vDualEdgesYm.empty()) lqueue.push_back(gridElemMeta.vDualEdgesYm[0]->elem0);
				if (!gridElemMeta.vDualEdgesYp.empty()) lqueue.push_back(gridElemMeta.vDualEdgesYp[0]->elem1);
				if (!gridElemMeta.vDualEdgesZm.empty()) lqueue.push_back(gridElemMeta.vDualEdgesZm[0]->elem0);
				if (!gridElemMeta.vDualEdgesZp.empty()) lqueue.push_back(gridElemMeta.vDualEdgesZp[0]->elem1);
	...
	PtrS<Mesh_Hexa> Mesh_GridCoarsen::GetFineSubmesh(int coarElemIdx)
	{
		...
		for (int i = 0; i < (int) mE.rows(); ++i)
			...
			{
				Node* pFineNode = m_pFineMesh->Nodes()[coarElemMeta.vfineElem[i]->Nodes()[j]->ID()];
				FineNodeMeta& fineNodeMeta = pFineNode->Trait<FineNodeMeta>(Tag::Tag_FineMeta);
				mE(i, j) = fineNodeMeta.mLocalActiveIdx[coarElemIdx];

				for (int k = 0; k < (int) vmN.size(); ++k)
					vmN[k].row(fineNodeMeta.mLocalActiveIdx[coarElemIdx]) = pFineNode->Trait<Vector3d>(this->m_vnodeTraits[k]);
			}

		// Build fine submesh

		PtrS<Mesh_Hexa> pFineSubmesh(new Mesh_Hexa(vmN[0], mE, this->m_meshType, this->m_vnodeTraits));
	...
	void Mesh_GridCoarsen::GetFineSubmesh(int coarElemIdx, MatrixXd& mN, MatrixXi& mE, Tag s)
	{
		CoarElemMeta& coarElemMeta = this->Elems()[coarElemIdx]->Trait<CoarElemMeta>(Tag::Tag_CoarMeta);
		...
		for (int i = 0; i < (int)mE.rows(); ++i)
			for (int j = 0; j < coarElemMeta.vfineElem[i]->NumNodes(); ++j)
			{
				Node* pFineNode = m_pFineMesh->Nodes()[coarElemMeta.vfineElem[i]->Nodes()[j]->ID()];
				FineNodeMeta& fineNodeMeta = pFineNode->Trait<FineNodeMeta>(Tag::Tag_FineMeta);
				mN.row(fineNodeMeta.mLocalActiveIdx[coarElemIdx]) = pFineNode->Trait<Vector3d>(s);
				mE(i, j) = fineNodeMeta.mLocalActiveIdx[coarElemIdx];
			}
	}
	void Mesh_GridCoarsen::GetOversamplingSubmesh(vector<int> vcoarElemIdx, MatrixXd& mN, MatrixXi& mE, Tag s)
	{	
