...
namespace PhySim
{
	...
	Mesh_Face::Mesh_Face(const MatrixXd& mV, const MatrixXi& mF, Discretization D, const vector<Tag>& vnTraits) : Mesh(mV, mF, D, vnTraits)
	{
		this->UpdateMetadata();
	}

	void Mesh_Face::Init(const MatrixXd& mV, const MatrixXi& mF, Discretization D, const vector<Tag>& vnTraits)
	{
	...
	void Mesh_Face::GetEdgeHinge_Nodes(const Edge* pEdge, vector<Node*>& vhinge)
	{
		...
		if (this->IsBoundary(pEdge))
		{
			...
			vhinge.push_back(pheHalf->m_pheNode->m_pHandle); pheHalf = pheHalf->m_pheNext;
			vhinge.push_back(pheHalf->m_pheNode->m_pHandle); pheHalf = pheHalf->m_pheNext;
	...
	void  Mesh_Face::GetNodeRing_InputHE(const Node* pNode, vector<HE_Half*>& vring)
	{
		...
		if (pheHalfCW == NULL)
		{
			...
			while (pheHalfCCW != NULL)
			{
				...
				do pheHalfCCW = pheHalfCCW->m_pheNext;
	...
	void  Mesh_Face::GetNodeRing_OutputHE(const Node* pNode, vector<HE_Half*>& vring)
	{
	...
	void Mesh_Face::GetNodeRing_Faces(const Node* pNode, vector<Face*>& vring, int D)
	{
		if (D != 1)
		{
	...
	void Mesh_Face::GetNodeRing_Edges(const Node* pNode, vector<Edge*>& vring, int D)
	{
		throw exception("Not implemented");
	}

	void Mesh_Face::GetNodeRing_Nodes(const Node* pNode, vector<Node*>& vring, int D)
	{
		throw exception("Not implemented");
	}
	...
	void Mesh_Face::ComputeFaceToNodeMap_Area(MatrixSd& mM, Tag s)
	{
		...
		for (int i = 0; i < (int)m_vnodes.size(); ++i)
		{
			...
			for (int j = 0; j < (int)vfaces.size(); ++j)
				vweiT.push_back(Triplet<Real>(m_vnodes[i]->ID(), vfaces[j]->ID(), vwei[j]));
		}
	...
	Matrix2d Mesh_Face::ComputeShapeOperator(const Face* pFace, Tag s)
	{
		...
		do
		{
			...
			Vector3d v_i = 
				phHalfCur0->m_pheNode->m_pHandle->Traits().Vector3d(s) -
	...
	Matrix2d Mesh_Face::ComputePlanarStrain(const Face* pFace, Tag s0, Tag sx, const VectorXd& vp)
	{
	...
	Matrix2d Mesh_Face::ComputeBendingStrain(const Face* pFace, Tag s0, Tag sx)
	{
		...
		do
		{
			...
			Vector3d v_i = 
				phHalfCur0->m_pheNode->m_pHandle->Traits().Vector3d(s0) -
				phHalfCur1->m_pheNode->m_pHandle->Traits().Vector3d(s0);
			v_i.normalize();
			Vector2d t_i = mA*(v_i.cross(n_i_0).normalized());

			S += (angle_i_x - angle_i_0)*pEdge_i->Length(s0)*(t_i*t_i.transpose());
	...
	bool Mesh_Face::IsBoundary(const Edge* pEdge)
	{
		HE_Edge* pheEdge = this->GetHEEdge(pEdge->ID());
		return 
			pheEdge->m_pheHalf0 == NULL ||
			pheEdge->m_pheHalf1 == NULL;
	}
	...
	bool Mesh_Face::IsBoundary(const Face* pFace, bool node)
	{
		...
		if (!hasBoundaryEdge && node)
		{
			...
			hasBoundaryNode |= IsBoundary(pheHalf->m_phePair->m_pheFace->m_pHandle, false);
			pheHalf = pheHalf->m_pheNext;
			hasBoundaryNode |= IsBoundary(pheHalf->m_phePair->m_pheFace->m_pHandle, false);
			pheHalf = pheHalf->m_pheNext;
			hasBoundaryNode |= IsBoundary(pheHalf->m_phePair->m_pheFace->m_pHandle, false);
		...
		for (int i = 0; i < this->m_vfaces.size(); ++i)
		{
			...
			for (int i = 0; i < NF; ++i)
			{
				...
				if (pheEdge == NULL)
				{
					...
					this->m_vedges.push_back(new Edge((int) this->m_vedges.size(), vedgeNodes));
				...
				else pheEdge->m_pheHalf1 = this->m_vheHalfs.back();
			}
		}
