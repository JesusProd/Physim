...
namespace PhySim
{
	...
	void Geometry::InterpolateEmbeddedNodeTraits(Tag s)
	{
		for (int i = 0; i < this->NumNodes(); ++i)
			if (this->Nodes()[i]->Traits().HasTrait(Tag::Tag_Embedding_0))
				this->Nodes()[i]->Traits().Vector3d(s) = this->Nodes()[i]->Trait<Embedding>(Tag::Tag_Embedding_0).InterpolateValue(s);
	}
	...
	void Geometry::InterpolateEmbeddedDeformation(Tag s0, Tag sx)
	{
		for (int i = 0; i < this->NumNodes(); ++i)
			if (this->Nodes()[i]->Traits().HasTrait(Tag::Tag_Embedding_0))
				this->Nodes()[i]->Traits().Vector3d(sx) = this->Nodes()[i]->Trait<Embedding>(Tag::Tag_Embedding_0).InterpolateDeformation(s0, sx);
	}

	void Geometry::ComputeEmbedding(vector<PtrS<Embedding>>& vembed, const VectorXi& vsel)
	{
		if (vsel.size() == 0)
		{
		...
		else
		{
			...
			{
				vembed[i] = make_shared<Embedding>(this->Nodes()[vsel[i]], VectorXd::Zero(1));
			}
		}
	}

	void Geometry::ComputeProjection(const MatrixXd& vpoints, vector<PtrS<Embedding>>& vembed, Tag s)
	{
	...
	void Geometry::ComputeEmbedding(const MatrixXd& vpoints, vector<PtrS<Embedding>>& vembed, Tag s)
	{
		vembed.resize(vpoints.rows());
	...
	template <class T> void Geometry::GetNodesTrait(vector<T>& vvalues, Tag s) const
	{
	...
	template <class T> void Geometry::SetNodesTrait(const vector<T>& vvalues, Tag s)
	{
		...
		for (int i = 0; i < numNodes; ++i)
		{
			...
			else m_vnodes[i]->Traits().Trait<T>(s) = vvalues[i];
		}
	}

	template <class T> void Geometry::GetElemsTrait(vector<T>& vvalues, Tag s) const
	{
	...
	template <class T> void Geometry::SetElemsTrait(const vector<T>& vvalues, Tag s)
	{
		...
		for (int i = 0; i < numElems; ++i)
		{
			...
			else m_velems[i]->Traits().Trait<T>(s) = vvalues[i];
		}
	}
