...
namespace PhySim
{
	...
	EnergyElement_Force::EnergyElement_Force(Simulable* pModel, const vector<KinematicsEle*>& vDoF) : EnergyElement(pModel)
	{
		int countDoF = 0;
		this->m_vDoF = vDoF;
	...
	void EnergyElement_Force::ComputeAndStore_Gradient_Internal()
	{
		...
		for (size_t i = 0; i < m_vDoF.size(); ++i)
		{
			m_vgradient.block(dofOffset, 0, m_vDoF[i]->NumDim(), 1) = this->m_vforces[i];
