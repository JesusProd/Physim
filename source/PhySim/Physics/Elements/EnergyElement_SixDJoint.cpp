...
EnergyElement_SixDJoint::EnergyElement_SixDJoint(Simulable* pModel,
                                                 PtrS<KEleRigidBody3D> pRBp,
                                                 PtrS<KEleRigidBody3D> pRBc,
                                                 MatrixXd K)
...
void EnergyElement_SixDJoint::Init() {
  ...
  if (m_jointCenter.norm() < 1e-6) {
    this->m_jointCenter = 0.5 * (m_vDoF[0]->GetValue().segment(0, 3) +
                                 m_vDoF[1]->GetValue().segment(0, 3));
  }
  m_rp = m_jointCenter - m_pRBp->GetPositionX().segment(0, 3);
  m_rc = m_jointCenter - m_pRBc->GetPositionX().segment(
                             0, 3);  // carefull here with the signs, not sure
                                     // if I should change something

  if (m_R0.norm() <
      1e-6) {  // calculate R0, orientation of the joints between vertebra
    ...
    if ((pos1 - pos2).norm() <= 1e-5) {
      ...
      newY = pos1 - pos2;  // here because of the two cubes next to each other,
                           // newX and newY are exactly the same
      newY = newY / newY.norm();  // this is not the case in the real case, so I
                                  // do not try to fix the problem now

      // std::cout << "newY = " << newY << std::endl;
      xvec = Vector3d(1.0, 0.0, 0.0);
      // std::cout<< "gia na doume edo" <<(xvec.transpose() * newY) * (1 /
      // (newY.norm() * newY.norm()))<<std::endl;
    ...
    rotmat.row(0) = newX;
    rotmat.row(1) = newY;
    rotmat.row(2) = newZ;  // CAREFULL here R0 is in global coordinates, in the
                           // paper it is in local so I need to transform it
    // std::cout << "rotation matrix rotmat" << rotmat << std::endl;
    Matrix3d Rp = m_pRBp->ComputeFullRotation();
    this->m_R0 = Rp.transpose() *
                 rotmat;  // here I transfer R0 to the parent coordinates
  ...
  // m_rc = Vector3d(-0.05, +0.05, +0.05);
  // need to add R0 here, R0 is the initial orientation of the frame. For the
  // moment I am passing it from outside but it can be computed here. It depends
  // on the center of masses of the rigids.
}
...
void EnergyElement_SixDJoint::UpdateMechanics() {
  ...
  m_xNodep = m_pRBp->GetPositionX().segment(0, 3) +
             m_Rp * m_rp;  // node of the joint belonging to parent
  m_xNodec = m_pRBc->GetPositionX().segment(0, 3) +
             m_Rc * m_rc;  // node of the joint belonging to child
}
...
void EnergyElement_SixDJoint::ComputeAndStore_Energy_Internal() {
  ...
  Vector3d dthita = GeometryUtils::rotationMatrixToAxisAngle(
      m_R0.transpose() * m_Rp.transpose() * m_Rc *
      m_R0);  // I need to transform this to axis angle

  this->m_energy = 0.5 * dx.transpose() * m_K.block(0, 0, 3, 3) * dx;
  this->m_energy += 0.5 * dthita.transpose() * m_K.block(3, 3, 3, 3) * dthita;
}

void EnergyElement_SixDJoint::
    ComputeAndStore_Energy_Internal_Only_Translation() {
  // Strain
  Vector3d dx = m_R0.transpose() * m_Rp.transpose() * (m_xNodec - m_xNodep);
  Vector3d dthita = GeometryUtils::rotationMatrixToAxisAngle(
      m_R0.transpose() * m_Rp.transpose() * m_Rc *
...
void EnergyElement_SixDJoint::ComputeAndStore_Gradient_Internal() {
  ...
  Vector3d dx = m_R0.transpose() * m_Rp.transpose() * (m_xNodec - m_xNodep);
  Vector3d dphi = GeometryUtils::rotationMatrixToAxisAngle(
      m_R0.transpose() * m_Rp.transpose() * m_Rc *
      m_R0);  // same as dthita above, change to be consistent with the axis
              // angle .doc
  ...
  dphi_hat << 0, -dphi(2), dphi(1), dphi(2), 0, -dphi(0), -dphi(1), dphi(0), 0;

  Vector3d Tc, Tp;
  MtoInv = phiInRotMat - I + dphi * dphi.transpose();
  if (MtoInv.norm() > 1e-8) {
    Matrix3d dphidthita =
        MtoInv.inverse() * (dphi_hat + dphi * dphi.transpose());
    Tp = -m_R0 * dphidthita.transpose() * m_K.block(3, 3, 3, 3) *
         dphi;  // - m_Rp * dphidthita.transpose() * m_K.block(3, 3, 3, 3) *
                // dphi;
    Tc = -Tp;
  ...
  // extra rotational part. derivative of eq 6 in paper with respect to rot dofs
  // inludes both parts
  ...
  skewThitap << 0, -thitap(2), thitap(1), thitap(2), 0, -thitap(0), -thitap(1),
      thitap(0), 0;

  skewRc0rc << 0, -rc_hat(2), rc_hat(1), rc_hat(2), 0, -rc_hat(0), -rc_hat(1),
      rc_hat(0), 0;

  // ddxdthitac = - m_R0.transpose() * Rp0.transpose() * (eye - skewThitap) *
  // skewRc0rc;
  ...
  rp_hat = Rp0 * m_rp;
  skewRp0rp << 0, -rp_hat(2), rp_hat(1), rp_hat(2), 0, -rp_hat(0), -rp_hat(1),
      rp_hat(0), 0;

  b = m_xNodec - m_xNodep;
  skewb << 0, -b(2), b(1), b(2), 0, -b(0), -b(1), b(0), 0;

  ddxdthitap = m_R0.transpose() * m_Rp.transpose() * skewRp0rp +
               m_R0.transpose() * Rp0.transpose() * skewb;
...
void EnergyElement_SixDJoint::
    ComputeAndStore_Gradient_Internal_Only_Translation() {
  // Strain
...
void EnergyElement_SixDJoint::ComputeAndStore_Hessian_Internal() {
  ...
  // Translational part
  // Matrix3d ddxdxc = m_R0.transpose() * m_Rp.transpose();
  // Matrix3d thc = ddxdxc.transpose() * m_K.block(0, 0, 3, 3) * ddxdxc;     //
  // need to think about this minus
  ////
  ...
  Rp0rp = Rp0 * m_rp;
  Rc0rc = Rc0 * m_rc;
  skewRp0rp << 0, -Rp0rp(2), Rp0rp(1), Rp0rp(2), 0, -Rp0rp(0), -Rp0rp(1),
      Rp0rp(0), 0;
  skewRc0rc << 0, -Rc0rc(2), Rc0rc(1), Rc0rc(2), 0, -Rc0rc(0), -Rc0rc(1),
      Rc0rc(0), 0;
  b = m_xNodec - m_xNodep;
  skewb << 0, -b(2), b(1), b(2), 0, -b(0), -b(1), b(0), 0;
  Rp0R0KDx = Rp0 * m_R0 * m_K.block(0, 0, 3, 3) * dx;
  skewRp0R0KDx << 0, -Rp0R0KDx(2), Rp0R0KDx(1), Rp0R0KDx(2), 0, -Rp0R0KDx(0),
  ...
  Matrix3d d2Edxcthitap = RpR0K * (R0tRpt * skewRp0rp +
                                   m_R0.transpose() * Rp0.transpose() * skewb) -
                          skewRp0R0KDx;
  ...
  Matrix3d d2Edthitacthitap =
      skewRc0rc * (-skewRp0R0KDx) +
      skewRc0rc * RpR0K *
          (R0tRpt * skewRp0rp + m_R0.transpose() * Rp0.transpose() * skewb);

  // d2Ethitapthitac = d2Ethitacthitap^t
  Matrix3d d2Edthitapthitap =
      -skewRp0rp * (-skewRp0R0KDx) + skewRp0R0KDx * skewRp0rp +
  ...
  if (MtoInv.norm() > 1e-8) {
    Matrix3d dphidthita =
        MtoInv.inverse() * (dphi_hat + dphi * dphi.transpose());
    // rhc = -m_Rp * dphidthita.transpose() * m_K.block(3, 3, 3, 3) * dphidthita
    // * (-m_Rp.transpose());
    rhc = -m_R0 * dphidthita.transpose() * m_K.block(3, 3, 3, 3) * dphidthita *
          (-m_R0.transpose());
