...
EnergyElement_DER_BendTwist::EnergyElement_DER_BendTwist(
    Simulable* pModel,
  ...
  this->m_vDoF[0] = vedges[0]->GetTail()->Traits().Kinematics(Tag::DOF_0);
  this->m_vDoF[1] = vedges[0]->GetHead()->Traits().Kinematics(Tag::DOF_0);
  this->m_vDoF[2] = vedges[1]->GetHead()->Traits().Kinematics(Tag::DOF_0);
  this->m_vDoF[3] = vedges[0]->Traits().Kinematics(Tag::DOF_0);
  this->m_vDoF[4] = vedges[1]->Traits().Kinematics(Tag::DOF_0);
...
void EnergyElement_DER_BendTwist::Init() {
  this->m_intVolume = 0.5 * this->m_vedges[0]->VolumeBasis(Tag::Position_0) +
                      0.5 * this->m_vedges[1]->VolumeBasis(Tag::Position_0);

  this->m_bend0 = this->ComputeBendStrain(Tag::Position_0);
  this->m_twist0 = this->ComputeTwistStrain(Tag::Position_0);
}

void EnergyElement_DER_BendTwist::ComputeAndStore_Energy_Internal() {
  // Get data

  Vector3d e1 = this->m_vedges[0]->Vector(Tag::Position_X);
  Vector3d e2 = this->m_vedges[1]->Vector(Tag::Position_X);

  const Frame3d& Fr_e = this->m_vedges[0]->Traits().Frame3d(Tag::Frame_X);
  const Frame3d& Fr_f = this->m_vedges[1]->Traits().Frame3d(Tag::Frame_X);
  const Real& t_e = this->m_vedges[0]->Traits().Double(Tag::Angle_X);
  const Real& t_f = this->m_vedges[1]->Traits().Double(Tag::Angle_X);
  const Real& rt = this->m_vedges[0]->GetHead()->Traits().Double(Tag::Twist_X);

  // Get material

  ParameterSet& parameters0 =
      *this->m_pModelDER->MaterialDistribution()->GetValueAtDomainPoint(
          this->m_vedges[0]->ID());
  ParameterSet& parameters1 =
      *this->m_pModelDER->MaterialDistribution()->GetValueAtDomainPoint(
          this->m_vedges[1]->ID());

  Real vshear[2] = {parameters0[ParameterSet::Param_Lame2],
                    parameters1[ParameterSet::Param_Lame2]};
  Real vyoung[2] = {parameters0[ParameterSet::Param_Young],
                    parameters1[ParameterSet::Param_Young]};

  // Get radius

  Real vradh[2] = {this->m_vedges[0]->Traits().Double(Tag::Size_0),
                   this->m_vedges[1]->Traits().Double(Tag::Size_0)};
  Real vradw[2] = {this->m_vedges[0]->Traits().Double(Tag::Size_1),
                   this->m_vedges[1]->Traits().Double(Tag::Size_1)};

  // Compute

  this->m_energy = getBTEnergyOPT(
      vshear, vyoung, vradw, vradh,
      this->m_vedges[0]->VolumeBasis(Tag::Position_0),
      this->m_vedges[1]->VolumeBasis(Tag::Position_0), this->m_bend0.data(),
...
void EnergyElement_DER_BendTwist::ComputeAndStore_Gradient_Internal() {
  ...
  getBTGradientOPT(vshear, vyoung, vradw, vradh,
                   this->m_vedges[0]->VolumeBasis(Tag::Position_0),
                   this->m_vedges[1]->VolumeBasis(Tag::Position_0),
  ...
  //	logTrace(Verbosity::V1_Default, "\n[DEBUG] Angle gradient E error: %.6e
  // vs %.6e", gt_e, m_vgradient[9]); 	logTrace(Verbosity::V1_Default,
...
Vector2d EnergyElement_DER_BendTwist::ComputeBendStrain(Tag s) {
  ...
  if (s == Tag::Position_0) {
    e = this->m_vedges[0]->Vector(Tag::Position_0);
    f = this->m_vedges[1]->Vector(Tag::Position_0);

    Fr_e = this->m_vedges[0]->Traits().Frame3d(Tag::Frame_0);
    Fr_f = this->m_vedges[1]->Traits().Frame3d(Tag::Frame_0);
    t_e = this->m_vedges[0]->Traits().Double(Tag::Angle_0);
    t_f = this->m_vedges[1]->Traits().Double(Tag::Angle_0);

    rt = 0;
  }

  if (s == Tag::Position_X) {
    e = this->m_vedges[0]->Vector(Tag::Position_X);
    f = this->m_vedges[1]->Vector(Tag::Position_X);

    Fr_e = this->m_vedges[0]->Traits().Frame3d(Tag::Frame_X);
    Fr_f = this->m_vedges[1]->Traits().Frame3d(Tag::Frame_X);
    t_e = this->m_vedges[0]->Traits().Double(Tag::Angle_X);
    t_f = this->m_vedges[1]->Traits().Double(Tag::Angle_X);

    rt = this->m_vedges[0]->GetHead()->Traits().Double(Tag::Twist_X);
  }
...
Real EnergyElement_DER_BendTwist::ComputeTwistStrain(Tag s) {
  ...
  if (s == Tag::Position_0) {
    t_e = this->m_vedges[0]->Traits().Double(Tag::Angle_0);  // Material twist
    t_f = this->m_vedges[1]->Traits().Double(Tag::Angle_0);  // Material twist
    rt = 0;
  }

  if (s == Tag::Position_X) {
    t_e = this->m_vedges[0]->Traits().Double(Tag::Angle_X);  // Material twist
    t_f = this->m_vedges[1]->Traits().Double(Tag::Angle_X);  // Material twist
    rt = this->m_vedges[0]->GetHead()->Traits().Double(
        Tag::Twist_X);  // Reference twist
  }
