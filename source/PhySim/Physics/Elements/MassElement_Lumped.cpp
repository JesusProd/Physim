...
namespace PhySim
{
	...
	MassElement_Lumped::MassElement_Lumped(Simulable* pModel, Poly* pPoly, PtrS<ParameterSet> pParams) : MassElement(pModel, pParams)
	{
		this->m_pPoly = pPoly;
	...
	void MassElement_Lumped::ComputeAndStore_Mass()
	{
		...
		this->m_mMass = MatrixXd::Identity(numDOF, numDOF)*mass/m_pPoly->NumNodes();
	}
	...
	void MassElement_Lumped::ComputeAndStore_Gradient_Internal()
	{
		...
		for (int i = 0; i < (int)m_vDoF.size(); ++i)
			this->m_vgradient.segment(3 * i, 3) = -this->m_mMass(i, i)*this->m_vgravity;
	}
