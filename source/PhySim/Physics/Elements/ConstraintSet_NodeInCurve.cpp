...
void ConstraintSet_NodeInCurve::ComputeAndStore_Values() {
  ...
  this->m_pCurve->ComputeProjection(vx.transpose(), ve, Tag::Position_0);

  const Edge* pEdge = static_cast<const Edge*>(ve[0]->Master());
  this->m_v0 = pEdge->GetTail()->Traits().Vector3d(Tag::Position_0);
  this->m_v1 = pEdge->GetHead()->Traits().Vector3d(Tag::Position_0);
  this->m_vt = pEdge->Tangent(Tag::Position_0);
