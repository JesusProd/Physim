...
namespace PhySim
{
	...
	EnergyElement_SpringBetwNodes::EnergyElement_SpringBetwNodes(Simulable* pModel, Node* node1, Node* node2, PtrS<ParameterSet> pMaterial) : EnergyElement(pModel)
	{
		this->m_node1 = node1;
		this->m_node2 = node2;
	...
	void EnergyElement_SpringBetwNodes::ComputeAndStore_Gradient_Internal()
	{
		...
		this->m_vgradient.segment(0, 3) = Vector3d(-s[0]*k_anisotropic(0, 0), -s[1]*k_anisotropic(1, 1), -s[2]*k_anisotropic(2, 2));
		this->m_vgradient.segment(3, 3) = Vector3d(s[0]*k_anisotropic(0, 0), s[1]*k_anisotropic(1, 1), s[2]*k_anisotropic(2, 2));
	}
