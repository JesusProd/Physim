...
void EnergyElement::PreprocessAssembly(int layer) {
  if (this->m_vgradient.size() == 0 && this->m_mHessian.size() == 0)
    return;  // Ignore assembly

  this->m_pAssembler->PreprocessMappings(
      vector<IDoFSet*>(this->m_vDoF.begin(), this->m_vDoF.end()), layer);
}
...
void EnergyElement::AssemblePreprocessedGradient(AVectorXd& vglobalGradient) {
  ...
  this->m_pAssembler->AssemblePreprocessedGradient(this->m_vgradient,
                                                   vglobalGradient);
}
...
void EnergyElement::AssemblePreprocessedHessian(AMatrixSd& mglobalHessian) {
  ...
  this->m_pAssembler->AssemblePreprocessedHessian(this->m_mHessian,
                                                  mglobalHessian);
}
...
void EnergyElement::Propagate_Gradient(AVectorXd& vglobalGradient) {
  ...
  this->m_pAssembler->PropagateGradient(
      this->m_vgradient,
      vector<IDoFSet*>(this->m_vDoF.begin(), this->m_vDoF.end()),
      vglobalGradient.Layer());
}
...
void EnergyElement::Propagate_Hessian(AMatrixSd& mglobalHessian) {
  ...
  this->m_pAssembler->PropagateHessian(
      this->m_mHessian,
      vector<IDoFSet*>(this->m_vDoF.begin(), this->m_vDoF.end()),
      mglobalHessian.Layer());
}
...
bool EnergyElement::TestLocalGradient() {
  ...
  if (realNorm < 1e-6) {
    ...
    if (diffNorm / realNorm > 1e-6) {
      ...
      // ostringstream str;
      // str << endl << "Analitic|Estimated|Difference:" << endl << mResult <<
      // endl; logTrace(Verbosity::V1_Default, "%s", str.str().c_str());
