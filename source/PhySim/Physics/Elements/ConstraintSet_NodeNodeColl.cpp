...
namespace PhySim
{
    ...
    ConstraintSet_NodeNodeColl::ConstraintSet_NodeNodeColl(Simulable* pModel, bool isSoft, Embedding* pEmbedA, Embedding* pEmbedB, Real tol, bool isTwoWay)
        ...
        , m_pEmbedB(pEmbedB)
    {
        ...
        assert(pMasterA->DimSpace() == 3 && pMasterB->DimSpace() == 3 && "Only 3-dimensional spaces are supported.");

        PtrS<ShapeFunction_LERP> pShapeFuncA = dynamic_pointer_cast<ShapeFunction_LERP>(pMasterA->GetShapeFunction());
        PtrS<ShapeFunction_LERP> pShapeFuncB = dynamic_pointer_cast<ShapeFunction_LERP>(pMasterB->GetShapeFunction());
        assert(pShapeFuncA && pShapeFuncB && "Only LERP-based shape functions are supported.");
		...
		if (this->m_isTwoWay)
		{
			...
			for (auto pNode : pMasterA->Nodes()) this->m_vDoFs.push_back(pNode->Traits().Kinematics(Tag_DOF_0));
			for (auto pNode : pMasterB->Nodes()) this->m_vDoFs.push_back(pNode->Traits().Kinematics(Tag_DOF_0));

			this->m_size = 1;
			this->m_vvalues.resize(1);
			this->m_mJacobian.resize(1, this->m_NumDoFsA + this->m_NumDoFsB);
			this->m_vHessian.resize(1);
			this->m_vHessian[0].resize(this->m_NumDoFsA + this->m_NumDoFsB, this->m_NumDoFsA + this->m_NumDoFsB);
		}
		else
		{
    ...
    void ConstraintSet_NodeNodeColl::ProjectConstraint()
    {
        ...
        const Real wA = Real(this->m_vDoFs[0]->Active() && !this->m_vDoFs[0]->GetFixed());
        const Real wB = Real(this->m_vDoFs[1]->Active() && !this->m_vDoFs[1]->GetFixed());
        if ((wA + wB) == 0.0) return;
    ...
    void ConstraintSet_NodeNodeColl::ComputeAndStore_Hessian()
    {
        ...
        const Matrix3d d2LdxAxA = (1 / L) * (Matrix3d::Identity() - dLdxA * dLdxA.transpose());
		...
		{
			HAA.block(i * 3, j * 3, 3, 3) = -(m_vParamsA[i] * m_vParamsA[j]) * d2LdxAxA;
		}
		...
		if (this->m_isTwoWay)
		{
			...
			auto HBB = this->m_vHessian[0].block(m_NumDoFsA, m_NumDoFsA, m_NumDoFsB, m_NumDoFsB);

			for (size_t i = 0; i < m_NumNodesB; ++i)
			for (size_t j = 0; j < m_NumNodesB; ++j)
			{
				HBB.block(i * 3, j * 3, 3, 3) = -(m_vParamsB[i] * m_vParamsB[j]) * d2LdxAxA;
			}

			for (size_t i = 0; i < m_NumNodesA; ++i)
			for (size_t j = 0; j < m_NumNodesB; ++j)
			{
				HAB.block(i * 3, j * 3, 3, 3) = (m_vParamsA[i] * m_vParamsB[j]) * d2LdxAxA;
				HBA.block(j * 3, i * 3, 3, 3) = (m_vParamsA[j] * m_vParamsB[i]) * d2LdxAxA;
			}
		}
    }
}