...
EnergyElement_FaceNodeColl_Quad::EnergyElement_FaceNodeColl_Quad(
  ...
  this->m_vDoF[0] = pFace->Nodes()[0]->Traits().Kinematics(Tag::DOF_0);
  this->m_vDoF[1] = pFace->Nodes()[1]->Traits().Kinematics(Tag::DOF_0);
  this->m_vDoF[2] = pFace->Nodes()[2]->Traits().Kinematics(Tag::DOF_0);
  this->m_vDoF[3] = pNode->Traits().Kinematics(Tag::DOF_0);
...
void EnergyElement_FaceNodeColl_Quad::ComputeAndStore_Energy_Internal() {
  //		const Vector3d& x0 =
  // this->m_vDoF[0]->Geometry()->Traits().Vector3d(Tag::Position_X);
  // const Vector3d& x1 =
  // this->m_vDoF[1]->Geometry()->Traits().Vector3d(Tag::Position_X);
  // const Vector3d& x2 =
  // this->m_vDoF[2]->Geometry()->Traits().Vector3d(Tag::Position_X);
  // const Vector3d& x3 =
  // this->m_vDoF[3]->Geometry()->Traits().Vector3d(Tag::Position_X);
  // Vector3d e0 = (x1 - x0).normalized(); 		Vector3d e1 = (x2 -
  // x0).normalized(); 		Vector3d e3 = x3 - x0; 		Real D =
  // e0.cross(e1).dot(e3); 		Real T =
