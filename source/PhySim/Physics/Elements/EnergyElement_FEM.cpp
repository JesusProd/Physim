...
EnergyElement_FEM::EnergyElement_FEM(Simulable_FEM* pSimul, Poly* pPoly)
    : EnergyElement(pSimul) {
...
void EnergyElement_FEM::UpdateKinematics() {
  this->m_pSimulFEM->SetupOptions()
      .m_pShapeFunctions->GetValueAtDomainPoint(this->m_pElemPoly->ID())
      ->UpdateKinematicsAtSamples(m_pSampleData);
}

void EnergyElement_FEM::UpdateMechanics() {
  this->m_pSimulFEM->SetupOptions()
      .m_pShapeFunctions->GetValueAtDomainPoint(this->m_pElemPoly->ID())
      ->UpdateDeformationAtSamples(m_pSampleData);
}
...
void EnergyElement_FEM::Init() {
  ...
  PtrS<IQuadrature> pQuad =
      this->m_pSimulFEM->SetupOptions().m_pQuadratures->GetValueAtDomainPoint(
          this->m_pElemPoly->ID());
...
void EnergyElement_FEM::ComputeAndStore_Energy_Internal() {
  ...
  for (int i = 0; i < pQuad->NumPoints(); ++i) {
    ...
    PtrS<IMaterialModel> pMatModel =
        this->m_pSimulFEM->SetupOptions().m_pMatModels->GetValueAtDomainPoint(
            this->m_pElemPoly->ID(), vp[i]);
    PtrS<ParameterSet> pMatParam =
        this->m_pSimulFEM->SetupOptions().m_pMatParams->GetValueAtDomainPoint(
            this->m_pElemPoly->ID(), vp[i]);

    // Vectorize DxD0 colmajor

    Real energyi = 0;
    pMatModel->ComputeEnergyDensity(*pMatParam, m_pSampleData->Get_F()[i],
                                    energyi);
...
MatrixXd EnergyElement_FEM::ComputeRestStrain() {
  return MatrixXd::Identity(this->m_pElemPoly->DimBasis(),
                            this->m_pElemPoly->DimBasis());
}
