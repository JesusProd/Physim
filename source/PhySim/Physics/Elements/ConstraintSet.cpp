...
namespace PhySim
{
	...
	bool ConstraintSet::IsActive()
	{
		...
		for (int i = 0; i < m_size; ++i)
		{
			switch (this->m_type)
			{
			case Type::EQ: if (this->m_vvalues[i] != 0) return true;
			case Type::LE: if (this->m_vvalues[i] >= 0) return true;
			case Type::LEQ: if (this->m_vvalues[i] > 0) return true;
			}
		}
	...
	void ConstraintSet::AssembleGlobal_Jacobian(VectorTd& vtotalTriplets)
	{
		...
		for (size_t j = 0; j < numDoFSet; ++j)
		{
			assembleSparseMatrix(
				...
				this->m_mJacobian.block(0, localOffset_j, this->m_size, this->m_vDoFs[j]->NumDim()));
