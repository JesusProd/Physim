...
namespace PhySim
{
	...
	EnergyElement_AlignFace::EnergyElement_AlignFace(Simulable* pModel, Face_Tri* pFace) : EnergyElement(pModel)
	{
		this->m_vDoF.resize(3);
		for (int i = 0; i < 3; ++i)
			this->m_vDoF[i] = pFace->Nodes()[i]->Traits().Kinematics(Tag_DOF_0);
	...
	void EnergyElement_AlignFace::Init()
	{
		...
		this->m_vnt = this->m_pFace->Normal(Tag_Position_X);
	...
	void EnergyElement_AlignFace::ComputeAndStore_Energy_Internal()
	{
		...
		MatrixXd mX;
		this->m_pFace->GetNodesTrait(mX, Tag_Position_X);
		...
		{
#include "../Maple/NormalAlignment_Energy.mcg";
...
#include "../Maple/NormalAlignment_Gradient.mcg";
...
#include "../Maple/NormalAlignment_Hessian.mcg";
