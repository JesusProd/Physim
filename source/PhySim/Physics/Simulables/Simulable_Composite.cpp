...
namespace PhySim
{
	...
	void Simulable_Composite::InitInternal()
	{
		...
		for (int i = 0; i < (int) this->m_pOptions->m_vDoFCouplings.size(); ++i)
		{
			...
			switch (type)
			{
			case DoFCoupling::Type::SoftFixed: addSoftFixedCoupling(this->m_pOptions->m_vDoFCouplings[i], vpMapped, vpRoots, vpEnergy); break;
			case DoFCoupling::Type::HardFixed: addHardFixedCoupling(this->m_pOptions->m_vDoFCouplings[i], vpMapped, vpRoots, vpEnergy); break;
			case DoFCoupling::Type::SoftEmbedding: addSoftEmbeddingCoupling(this->m_pOptions->m_vDoFCouplings[i], vpMapped, vpRoots, vpEnergy); break;
			case DoFCoupling::Type::HardEmbedding: addHardEmbeddingCoupling(this->m_pOptions->m_vDoFCouplings[i], vpMapped, vpRoots, vpEnergy); break;
			case DoFCoupling::Type::RigidBody: addRigidBodyCoupling(this->m_pOptions->m_vDoFCouplings[i], vpMapped, vpRoots, vpEnergy); break;
			}
		}
		...
		for (int i = 0; i < (int)this->m_pOptions->m_vpSimulables.size(); ++i)
		{
			PtrS<KinematicsTree> pTree = this->m_pOptions->m_vpSimulables[i]->GetKinematicsTree();
		...
		this->m_venerEle.insert(this->m_venerEle.end(), vpEnergy.begin(), vpEnergy.end());
	}
	...
	void Simulable_Composite::addSoftFixedCoupling(DoFCoupling& coupling, vector<PtrS<KinematicsEle>>& vpMapped, vector<PtrS<KinematicsEle>>& vpAdded, vector<PtrS<EnergyElement>>& vpEnergy)
	{
		// TODO
	}

	void Simulable_Composite::addHardFixedCoupling(DoFCoupling& coupling, vector<PtrS<KinematicsEle>>& vpMapped, vector<PtrS<KinematicsEle>>& vpAdded, vector<PtrS<EnergyElement>>& vpEnergy)
	{
		assert(coupling.m_type == DoFCoupling::Type::HardFixed);

		PtrS<KinematicsTree> pTree0 = this->m_pOptions->m_vpSimulables[coupling.m_mC(0, 0)]->GetKinematicsTree();
		PtrS<KinematicsTree> pTree1 = this->m_pOptions->m_vpSimulables[coupling.m_mC(1, 0)]->GetKinematicsTree();
	...
	void Simulable_Composite::addRigidBodyCoupling(DoFCoupling& coupling, vector<PtrS<KinematicsEle>>& vpMapped, vector<PtrS<KinematicsEle>>& vpAdded, vector<PtrS<EnergyElement>>& vpEnergy)
	{
		assert(coupling.m_type == DoFCoupling::Type::RigidBody);

		PtrS<KinematicsTree> pTreeRigid = this->m_pOptions->m_vpSimulables[coupling.m_mC(0, 0)]->GetKinematicsTree();
		PtrS<KinematicsTree> pTreeOther = this->m_pOptions->m_vpSimulables[coupling.m_mC(0, 2)]->GetKinematicsTree();
		
		// Get rigid body
		PtrS<KinematicsEle> pEleRigid = pTreeRigid->RootElements()[coupling.m_mC(0, 1)];
		...
		(new KMapRB2Particle3D())->Init(this, dynamic_pointer_cast<KEleRigidBody3D>(pNewRigid), vpEleOther);
		vpMapped.insert(vpMapped.end(), vpEleOther.begin(), vpEleOther.end());
	}

	void Simulable_Composite::addSoftEmbeddingCoupling(DoFCoupling& coupling, vector<PtrS<KinematicsEle>>& vpMapped, vector<PtrS<KinematicsEle>>& vpAdded, vector<PtrS<EnergyElement>>& vpEnergy)
	{
		// TODO
	}

	void Simulable_Composite::addHardEmbeddingCoupling(DoFCoupling& coupling, vector<PtrS<KinematicsEle>>& vpMapped, vector<PtrS<KinematicsEle>>& vpAdded, vector<PtrS<EnergyElement>>& vpEnergy)
	{
		// TODO
	}

	vector<IDoFSet*> Simulable_Composite::SelectDoF(const Vector3d& vboxMin, const Vector3d& vboxMax, Tag s) const
	{
		vector<IDoFSet*> vDoFs;

		for (int i = 0; i < (int)this->m_pOptions->m_vpSimulables.size(); ++i)
		{
			vector<IDoFSet*> vDoFs_i = this->m_pOptions->m_vpSimulables[i]->SelectDoF(vboxMin, vboxMax, s);
			vDoFs.insert(vDoFs.end(), 
						 vDoFs_i.begin(),
						 vDoFs_i.end());
		}
	...
	vector<PtrS<Geometry>> Simulable_Composite::Geometries() const
	{
		...
		for (int i = 0; i < (int)this->m_pOptions->m_vpSimulables.size(); ++i)
		{
			vector<PtrS<Geometry>> vpGeometries_i = m_pOptions->m_vpSimulables[i]->Geometries();
			vpGeometries.insert(vpGeometries.end(), vpGeometries_i.begin(), vpGeometries_i.end());
		}
