...
namespace PhySim
{
	...
	void Simulable_Mesh::CreateConstraintSets(vector<IConstraintSet*>& vConstraints)
	{
		// Nothing to do here... to be implemented by derived classes
	}
	...
	void Simulable_Mesh::CreateMassElements(vector<IMassElement*>& vMasses)
	{
		...
		{
			PtrS<ParameterSet> pPar = m_pOptions->m_pMatParams->GetValueAtDomainPoint(i);
	...
	vector<IDoFSet*> Simulable_Mesh::SelectDoF(const Vector3d& vboxMin, const Vector3d& vboxMax, Tag s) const
	{
		...
		for (size_t i = 0; i < this->m_pMesh->NumNodes(); ++i)
		{
			...
			if (pos.x() > vboxMin.x() && pos.x() < vboxMax.x() &&
				...
				vDoFs.push_back(this->m_pMesh->Nodes()[i]->Traits().Kinematics(Tag::Tag_DOF_0));
		}
