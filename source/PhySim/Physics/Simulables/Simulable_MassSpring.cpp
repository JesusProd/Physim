...
namespace PhySim
{
	...
	void Simulable_MassSpring::CreateEnergyElements(vector<IEnergyElement*>& vEnergies)
	{
		...
		this->m_venerEle.reserve(this->m_venerEle.size() + numLinear + numCrosses + numHinges);
		...
		if (numLinear > 0) { ; assert(pOptions->m_material.HasParameter(ParameterSet::Param_StretchK)); }
		if (numHinges > 0) { ; assert(pOptions->m_material.HasParameter(ParameterSet::Param_BendingK)); }
		if (numCrosses > 0) { ; assert(pOptions->m_material.HasParameter(ParameterSet::Param_ShearK)); }
		...
		{
			m_vmat_linear[i].AddParameter(ParameterSet::Param_StretchK, pOptions->m_material[ParameterSet::Param_StretchK]);
			m_vmat_linear[i].AddParameter(ParameterSet::Param_Density, pOptions->m_material[ParameterSet::Param_Density]);

			EnergyElement_SpringLinear* pEle = new EnergyElement_SpringLinear(this, (Edge*) this->m_pMesh->Elems()[m_vlinear(i)], &this->m_vmat_linear[i]);
		...
		{
			m_vmat_hinge[i].AddParameter(ParameterSet::Param_BendingK, pOptions->m_material[ParameterSet::Param_BendingK]);

			EnergyElement_SpringHinge* pEle = new EnergyElement_SpringHinge(this, (Edge*) this->m_pMesh->Elems()[m_mhinges(i, 0)], (Edge*) this->m_pMesh->Elems()[m_mhinges(i, 1)], &this->m_vmat_hinge[i]);
			vEnergies.push_back(pEle);
		...
		{
			m_vmat_cross[i].AddParameter(ParameterSet::Param_ShearK, pOptions->m_material[ParameterSet::Param_ShearK]);

			EnergyElement_SpringCross* pEle = new EnergyElement_SpringCross(this, (Edge*) this->m_pMesh->Elems()[m_mcrosses(i, 0)], (Edge*) this->m_pMesh->Elems()[m_mcrosses(i, 1)], &this->m_vmat_cross[i]);
			vEnergies.push_back(pEle);
	...
	VectorXd Simulable_MassSpring::GetStretchK() const
	{
		...
		for (size_t i = 0; i < numLinear; ++i)
		{
			vs[i] = (*this->m_venergyEle_linear[i]->GetMaterial())[ParameterSet::Param_StretchK];
		}
	...
	VectorXd Simulable_MassSpring::GetBendingK() const
	{
		...
		for (size_t i = 0; i < numHinges; ++i)
		{
			vs[i] = (*this->m_venergyEle_hinge[i]->GetMaterial())[ParameterSet::Param_BendingK];
		}
	...
	VectorXd Simulable_MassSpring::GetShearK() const
	{
		...
		for (size_t i = 0; i < numCrosses; ++i)
		{
			vs[i] = (*this->m_venergyEle_cross[i]->GetMaterial())[ParameterSet::Param_ShearK];
		}
	...
	void Simulable_MassSpring::SetStretchK(const VectorXd& vs)
	{
		...
		for (size_t i = 0; i < numLinear; ++i)
		{
			(*this->m_venergyEle_linear[i]->GetMaterial())[ParameterSet::Param_StretchK] = vs[i];
		}
	...
	void Simulable_MassSpring::SetBendingK(const VectorXd& vs)
	{
		...
		for (size_t i = 0; i < numHinges; ++i)
		{
			(*this->m_venergyEle_hinge[i]->GetMaterial())[ParameterSet::Param_BendingK] = vs[i];
		}
	...
	void Simulable_MassSpring::SetShearK(const VectorXd& vs)
	{
		...
		for (size_t i = 0; i < numCrosses; ++i)
		{
			(*this->m_venergyEle_cross[i]->GetMaterial())[ParameterSet::Param_ShearK] = vs[i];
		}
