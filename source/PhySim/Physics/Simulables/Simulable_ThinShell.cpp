...
void Simulable_ThinShell::CreateEnergyElements(
  ...
  assert(
      this->SetupOptions().m_material.HasParameter(ParameterSet::Param_Young));
  assert(this->SetupOptions().m_material.HasParameter(
      ParameterSet::Param_Poisson));
  assert(this->SetupOptions().m_material.HasParameter(
      ParameterSet::Param_Thickness));
  ...
  for (int i = 0; i < numEdges; ++i) {
    ...
    // pEdge->Traits().AddTrait<ParameterSet>(Semantics::Tag_Mat_0,
    // ParameterSet());
    ...
    /*			pEdge->Traits().ParameterSet(Semantics::Tag_Mat_0).AddParameter(ParameterSet::Param_Young,
    Y);
    pEdge->Traits().ParameterSet(Semantics::Tag_Mat_0).AddParameter(ParameterSet::Param_Poisson,
    v);
    pEdge->Traits().ParameterSet(Semantics::Tag_Mat_0).AddParameter(ParameterSet::Param_Thickness,
    t)*/
    ;
    ...
    if (pOptions->m_material.HasParameter(ParameterSet::Param_BendingK)) {
      ...
      // pEdge->Traits().ParameterSet(Tag_Mat_0).AddParameter(ParameterSet::Param_BendingK,
      // kB);
    ...
    EnergyElement_DiscreteShells* pEle =
        new EnergyElement_DiscreteShells(this, pEdge);
...
void Simulable_ThinShell::ComputePlanarStrain(vector<vector<Matrix2d>>& vmEp) {
  ...
  for (int i = 0; i < pMesh->NumElems(); ++i) {
    int numQ = this->SetupOptions()
                   ...
                   ->NumPoints();
    const vector<VectorXd>& vp =
        this->SetupOptions().m_pQuadratures->GetValueAtDomainPoint(i)->Points();

    vmEp[i].resize(numQ);
    for (int j = 0; j < numQ; ++j)
      vmEp[i][j] = pMesh->ComputePlanarStrain(pMesh->Faces()[i], Tag_Position_0,
                                              Tag_Position_X, vp[j]);
  }
}
...
void Simulable_ThinShell::ComputeNodalBendingStress(vector<Matrix2d>& vmSb) {
  ...
  Real thickness =
      this->SetupOptions().m_material[ParameterSet::Param_Thickness];
...
void Simulable_ThinShell::ComputeTotalStrain(vector<vector<Matrix2d>>& vmEt) {
  ...
  for (int i = 0; i < pMesh->NumElems(); ++i) {
    ...
    for (int j = 0; j < numQ; ++j) {
      ...
      vmEt[i][j] +=
          (thickness / 2) * svd.matrixU() * S * svd.matrixV().transpose();
    }
  }
}

void Simulable_ThinShell::ComputeStressIntegral(
    const vector<vector<Matrix2d>>& vmE,
    vector<Matrix2d>& vmS) {
  Mesh_Face* pMesh = MeshFace();
  ...
  for (int i = 0; i < pMesh->NumElems(); ++i) {
    ...
    PtrS<EnergyElement_FEM> pEle =
        dynamic_pointer_cast<EnergyElement_FEM>(m_venerEle[i]);
    ...
    for (int j = 0; j < numQ; ++j)
      vmS[i] +=
          this->ComputeStressForE(vmE[i][j]) * pEle->GetIntegrationWeights()[j];
  }
}

void Simulable_ThinShell::ComputeStressDensity(
    const vector<vector<Matrix2d>>& vmE,
    vector<Matrix2d>& vmS) {
  Mesh_Face* pMesh = MeshFace();
