...
void Simulable_DER::CreateKinematicsEle(vector<PtrS<KinematicsEle>>& vpEle) {
  ...
  vpEle.reserve(vpEle.size() + m_pMeshDER->NumNodes() + m_pMeshDER->NumEdges() +
                5 * m_pMeshDER->NumConns());
  ...
  for (int i = 0; i < m_pMeshDER->NumElems(); ++i) {
    ...
    m_pMeshDER->Elems()[i]->Traits().AddTrait<IDoFSet*>(Tag_DOF_0,
                                                        vpEle.back().get());
  }
  ...
  for (int i = 0; i < m_pMeshDER->NumNodes(); ++i) {
    ...
    m_pMeshDER->Nodes()[i]->Traits().AddTrait<IDoFSet*>(Tag_DOF_2,
                                                        vpEle.back().get());
  }
  ...
  for (int i = 0; i < m_pMeshDER->NumConns(); ++i) {
    ...
    for (int j = 0; j < (int)pConn->m_vedges.size(); ++j) {
      ...
      pConn->m_vedges[j]->Traits().AddTrait<IDoFSet*>(Tag_DOF_1,
                                                      vpEle.back().get());
    }
  }
}
...
void Simulable_DER::CreateEnergyElements(vector<IEnergyElement*>& vEle) {
  ...
  for (int i = 0; i < numEdges; ++i) {
    this->m_venerEle_stretch.push_back(
        new EnergyElement_DER_Stretch(this, this->m_pMeshDER->GetEdge(i)));
  ...
  for (int i = 0; i < numNodes; ++i) {
    ...
    if (m_pMeshDER->IsConnectionNode(pNode->ID()) || vprev.size() != 1 ||
        vnext.size() != 1)
    ...
    vedges[0] = vprev[0];
    vedges[1] = vnext[0];
    this->m_venerEle_bendTwist.push_back(
        new EnergyElement_DER_BendTwist(this, vedges));
  ...
  for (int i = 0; i < numConns; ++i) {
    ...
    for (int j = 0; j < numConnEdges; ++j) {
      this->m_venerEle_connection.push_back(new EnergyElement_DER_Connection(
          this, pConn->m_center, pConn->m_vedges[j]));
...
void Simulable_DER::CreateMassElements(vector<IMassElement*>& vMasses) {
  ...
  for (int i = 0; i < numEdges; ++i) {
    PtrS<ParameterSet> pPar =
        this->m_pOptions->m_pMatParams->GetValueAtDomainPoint(i);
    vMasses.push_back(
        new MassElement_DEREdge(this, this->m_pMeshDER->GetEdge(i), pPar));
  }
}
