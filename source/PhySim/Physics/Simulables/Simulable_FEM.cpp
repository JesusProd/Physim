...
namespace PhySim
{
	...
	void Simulable_FEM::PreInit()
	{
		...
		if (this->SetupOptions().m_pMatModels == NULL)
		{
			...
			this->SetupOptions().m_pMatModels.reset(new DomainDistribution_Constant<PtrS<IMaterialModel>>(pMatModel));
		}
		...
		if (this->SetupOptions().m_pShapeFunctions == NULL)
		{
			...
			this->SetupOptions().m_pShapeFunctions.reset(new DomainDistribution_Constant<PtrS<IShapeFunction>>(pShapeFunction));
		}
		...
		if (this->SetupOptions().m_pQuadratures == NULL)
		{
			...
			this->SetupOptions().m_pQuadratures.reset(new DomainDistribution_Constant<PtrS<IQuadrature>>(pQuadrature));
		}
	}
	...
	void Simulable_FEM::CreateEnergyElements(vector<IEnergyElement*>& venerEle)
	{
		...
		{
			this->m_venergyEle_fem[i] = new EnergyElement_FEM(this, this->m_pMesh->Elems()[i]);
