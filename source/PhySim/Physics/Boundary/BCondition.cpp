...
namespace PhySim
{
	...
	bool BCondition::ResetLoading()
	{
        ...
        {
            IOUtils::logTrace(PhySim::Verbosity::V1_Default, "\n[TRACE] BC %s loading (Continuous)", this->Name().c_str());
            return true;
        }
        else
        {
            ...
            IOUtils::logTrace(PhySim::Verbosity::V1_Default, "\n[TRACE] BC %s loading (Full): (%d/%d)", this->Name().c_str(),
                this->m_numSubsteps*this->CurStage() + this->m_curSubsteps, this->m_numSubsteps*this->NumStage());
	...
	bool BCondition::StepLoading()
	{
        ...
        else
        {
		    ...
		    IOUtils::logTrace(PhySim::Verbosity::V1_Default, "\n[TRACE] BC %s loading (Step): (%d/%d)", this->Name().c_str(),
			         this->m_numSubsteps*this->CurStage() + this->m_curSubsteps + 1, this->m_numSubsteps*this->NumStage());
