...
BC_Force::BC_Force(Simulable* pModel,
                   const vector<KinematicsEle*>& vpDoF,
                   const vector<VectorXd>& vforce)
    : BCondition(pModel) {
...
void BC_Force::Init() {
  ...
  this->m_pEleForce = PtrS<EnergyElement_Force>(
      new EnergyElement_Force(this->m_pModel, this->m_vpDoF));
