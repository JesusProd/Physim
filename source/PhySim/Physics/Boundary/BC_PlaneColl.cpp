...
namespace PhySim
{
	...
	BC_PlaneColl::BC_PlaneColl(Simulable* pModel, const Vector3d& vp, const Vector3d& vn, double tol, bool isSoft) : BCondition(pModel)
	{
		this->m_vpoints = vp;
		this->m_vn = vn;
		m_vn.normalize();
		...
		this->m_pFilter.reset(new GeometryFilter_PlaneDistNode(Tag_Position_X, vp, vn, Vector2d(-HUGE_VAL, tol)));
	}
	...
	void BC_PlaneColl::Update()
	{
		...
		for (int i = 0; i < vnodes.size(); ++i)
		{
			...
			PtrS<ConstraintSet> pCon(new ConstraintSet_NodePlaneColl(m_pModel, this->m_isSoft, pDOF, this->m_vpoints, this->m_vn, m_tol));
			if (this->m_isSoft) m_vpEne.push_back(PtrS<IEnergyElement>(new EnergyElement_SoftConstraint(this->m_pModel, pCon, 1.0e9)));
			else this->m_vpCon.push_back(pCon);
		}
	}
}
