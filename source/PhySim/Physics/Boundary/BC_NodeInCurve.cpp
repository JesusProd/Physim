...
BC_NodeInCurve::BC_NodeInCurve(Simulable* pModel,
  ...
  for (int i = 0; i < this->m_pCurve->NumNodes(); ++i)
    this->m_vini[i] = pCurve->Nodes()[i]->Traits().Vector3d(Tag::Position_0);
}
...
void BC_NodeInCurve::Init() {
  ...
  for (size_t i = 0; i < numP; ++i) {
    ...
    PtrS<ConstraintSet> pCon(
        new ConstraintSet_NodeInCurve(m_pModel, true, pDOF, this->m_pCurve));
  ...
  if (this->m_isSoft) {
    ...
    for (size_t i = 0; i < numP; ++i) {
      PtrS<ConstraintSet> pCon =
          dynamic_pointer_cast<ConstraintSet>(this->m_vpCon[i]);
      PtrS<EnergyElement> pEne(new EnergyElement_SoftConstraint(
          this->m_pModel, pCon, this->m_kAini));
...
void BC_NodeInCurve::Update() {
  ...
  for (int i = 0; i < vval.size(); ++i) {
    this->m_pCurve->Nodes()[i]->Traits().Vector3d(Tag::Position_0) = vval[i];
  }
