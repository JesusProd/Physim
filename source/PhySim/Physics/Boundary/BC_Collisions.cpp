...
namespace PhySim
{
    BC_Collisions::BC_Collisions(Simulable* pSimulable, const vector<PtrS<Collider>>& vColliders)
        : BCondition(pSimulable)
        , m_vColliders(vColliders)
    {
    }
    ...
    void BC_Collisions::Init()
    {
        ...
        for (int j = i; j < m_vColliders.size(); ++j)
        {
            ...
            if (it != m_ContactStorage.end()) continue;
    ...
    void BC_Collisions::Update()
    {
      IOUtils::logTrace(V1_Default, "\n[TRACE] Detecting collisions...");
        ...
        for (int j = i; j < m_vColliders.size(); ++j)
        {
            ...
            {
                DetectContacts(dynamic_pointer_cast<Collider_SphereCloud>(m_vColliders[i]),
        ...
        for (int j = i; j < m_vColliders.size(); ++j)
        {
            ...
            m_vpCon.insert(m_vpCon.end(), rContactSet.vpConstraints.begin(), rContactSet.vpConstraints.end());
            m_vpEne.insert(m_vpEne.end(), rContactSet.vpEnergyElements.begin(), rContactSet.vpEnergyElements.end());
        }
    ...
    void BC_Collisions::DetectContacts(const PtrS<Collider_SphereCloud>& pColliderA, const PtrS<Collider_SphereCloud>& pColliderB,
                                       vector<Contact>& vContactsA, vector<Contact>& vContactsB)
    {
        ...
        // Broad-Phase: Use acceleration structure to find all potential contact candidates.
        ...
        for (auto& Candidate : vCandidates)
        {
            ...
            Real Distance = (PositionA - PositionB).squaredNorm() - TotalRadius * TotalRadius;
            if (Distance >= 0.0) continue;

            // Special case: If self-colliding, make sure to prevent generating contacts with
            // adjacent neighbors.
            if (IsSelfCollision)
            {
                VectorXd RestPositionA = pPointA.Position->InterpolateValue(Tag_Position_0);
                VectorXd RestPositionB = pPointB.Position->InterpolateValue(Tag_Position_0);

                Real RestDistance = (RestPositionA - RestPositionB).squaredNorm() - TotalRadius * TotalRadius;
                if (RestDistance < 0.0) continue;
            }
    ...
    void BC_Collisions::DetectContacts(const PtrS<Collider_DistanceField>& pColliderA, const PtrS<Collider_SphereCloud>& pColliderB,
                                       vector<Contact>& vContactsA, vector<Contact>& vContactsB)
    {
    ...
    {
        assert(rContactSet.vContactsA.size() == rContactSet.vContactsB.size() && "Number of contacts mismatch!");
        ...
        if (TypeA == ColliderType_SphereCloud && TypeB == ColliderType_SphereCloud)
        {
            PtrS<Collider_SphereCloud> pSphereCloudA = dynamic_pointer_cast<Collider_SphereCloud>(rContactSet.pColliderA);
            PtrS<Collider_SphereCloud> pSphereCloudB = dynamic_pointer_cast<Collider_SphereCloud>(rContactSet.pColliderB);

            const vector<Collider_SphereCloud::Point>& vpPointsA = pSphereCloudA->Points();
            const vector<Collider_SphereCloud::Point>& vpPointsB = pSphereCloudB->Points();

            for (size_t i = 0; i < N; ++i)
            {
                const Collider_SphereCloud::Point& pPointA = vpPointsA[rContactSet.vContactsA[i].Index];
                const Collider_SphereCloud::Point& pPointB = vpPointsB[rContactSet.vContactsB[i].Index];
                ...
                if (rContactSet.pColliderA->IsSimulated() && rContactSet.pColliderB->IsSimulated()) // Two-way (both colliders simulated)
                    pConstraint = NewS<ConstraintSet_NodeNodeColl>(m_pModel, true, pPointA.Position, pPointB.Position, RadiusA + RadiusB, true);
                else if (rContactSet.pColliderA->IsSimulated()) // One-way (collider A is simulated)
                    pConstraint = NewS<ConstraintSet_NodeNodeColl>(m_pModel, true, pPointA.Position, pPointB.Position, RadiusA + RadiusB, false);
                else if (rContactSet.pColliderB->IsSimulated()) // One-way (collider B is simulated)
                    pConstraint = NewS<ConstraintSet_NodeNodeColl>(m_pModel, true, pPointB.Position, pPointA.Position, RadiusA + RadiusB, false);

                if (pConstraint != NULL)
                {
                    PtrS<EnergyElement_SoftConstraint> pEnergy = NewS<EnergyElement_SoftConstraint>(m_pModel, pConstraint, 1e9*ApproxSurfaceArea, true); // TODO: Expose stiffness.
    ...
    bool BC_Collisions::AbleToCollide(const PtrS<Collider>& pColliderA, const PtrS<Collider>& pColliderB)
    {
        ...
        return m_CollisionMatrix(CollisionLayerA, CollisionLayerB)
            && m_sFilteredColliders.find({ pColliderA.get(), pColliderB.get() }) == m_sFilteredColliders.end();
    }

    void BC_Collisions::EnableCollision(const PtrS<Collider>& pColliderA, const PtrS<Collider>& pColliderB)
    {
        auto it = m_sFilteredColliders.find({ pColliderA.get(), pColliderB.get() });
        if (it != m_sFilteredColliders.end()) m_sFilteredColliders.erase(it);
    }

    void BC_Collisions::DisableCollision(const PtrS<Collider>& pColliderA, const PtrS<Collider>& pColliderB)
    {
        auto it = m_sFilteredColliders.find({ pColliderA.get(), pColliderB.get() });
        if (it == m_sFilteredColliders.end()) m_sFilteredColliders.emplace(pColliderA.get(), pColliderB.get());
    }

    BC_Collisions::ContactSet* BC_Collisions::Contacts(const PtrS<Collider>& pColliderA, const PtrS<Collider>& pColliderB) const
    {
        auto it = m_ContactStorage.find({ pColliderA.get(), pColliderB.get() });
    ...
    vector<BC_Collisions::ContactSet*> BC_Collisions::Contacts(const PtrS<Collider>& pCollider) const
    {
