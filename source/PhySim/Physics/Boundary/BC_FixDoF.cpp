...
BC_FixDoF::BC_FixDoF(Simulable* pModel,
                     const vector<IDoFSet*>& vpDoF,
                     const vector<VectorXd>& vt,
                     bool isSoft,
                     Real kAini,
                     Real kAend)
    : BCondition(pModel) {
  ...
  for (size_t i = 0; i < numP; ++i) {
    if (!vt.empty())
      ...
      this->m_vini[i] = vpDoF[i]->GetValue();
  }
}
...
void BC_FixDoF::Init() {
  ...
  if (this->m_isSoft) {
    ...
    for (size_t i = 0; i < numP; ++i) {
      PtrS<ConstraintSet> pCon(new ConstraintSet_FixedPosition(
          m_pModel, (KinematicsEle*)m_vpDoF[i], m_vini[i]));
...
void BC_FixDoF::Update() {
  ...
  if (this->m_isSoft) {
    ...
    for (int i = 0; i < (int)m_vpDoF.size(); ++i) {
      ConstraintSet_FixedPosition* pCon =
          static_cast<ConstraintSet_FixedPosition*>(this->m_vpCon[i].get());
