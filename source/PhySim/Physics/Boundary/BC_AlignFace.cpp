...
namespace PhySim
{
	...
	BC_AlignFace::BC_AlignFace(Simulable* pModel, const vector<Face*>& vpFaces, const vector<Vector3d>& vn, bool isSoft, Real kAini, Real kAend) : BCondition(pModel)
	{
		...
		for (int i = 0; i < numP; ++i)
		{
			if (!vn.empty())
				this->m_vini[i] = vn[i];
			else this->m_vini[i] = vpFaces[i]->Normal(Tag_Position_X);
		}
	}
	...
	void BC_AlignFace::Init()
	{
		...
		if (this->m_isSoft)
		{
			...
			{
				PtrS<ConstraintSet> pCon(new ConstraintSet_AlignFace(m_pModel, m_vpFace[i], m_vini[i], true));
		...
		else
		{
			...
			for (size_t i = 0; i < numP; ++i)
			{
				this->m_vpCon.push_back(PtrS<ConstraintSet>(new ConstraintSet_AlignFace(m_pModel, m_vpFace[i], m_vini[i], false)));
			}
		}
	}
	...
	void BC_AlignFace::Update()
	{
		...
		{
			ConstraintSet_AlignFace* pCon = static_cast<ConstraintSet_AlignFace*>(this->m_vpCon[i].get());
