...
LayerMask::LayerMask() : m_BitMask(~0ull) {}

LayerMask::LayerMask(size_t bitMask) : m_BitMask(bitMask) {}
...
void LayerMask::Add(Layer layer) {
  assert(layer >= 0ull && layer <= 63ull &&
         "Specified layer is outside of the 0..63 range");
...
LayerMaskMatrix::LayerMaskMatrix() : LayerMaskMatrix(true) {}
...
size_t LayerMaskMatrix::IndexOf(Layer lhsLayer, Layer rhsLayer) const {
  assert(lhsLayer >= 0ull && lhsLayer <= 63ull &&
         "Specified layer is outside of the 0..63 range");
  assert(rhsLayer >= 0ull && rhsLayer <= 63ull &&
         "Specified layer is outside of the 0..63 range");

  return lhsLayer <= rhsLayer ? (128 - lhsLayer - 1) * lhsLayer / 2 + rhsLayer
