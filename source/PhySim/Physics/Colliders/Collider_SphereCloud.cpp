...
namespace PhySim
{
    ...
    Collider_SphereCloud::Collider_SphereCloud(Geometry* pGeom, const vector<SurfaceSample>& vSamples)
    {
        Init(pGeom, vSamples);
        this->m_pModel = NULL;
    }

    Collider_SphereCloud::Collider_SphereCloud(Simulable* pModel, const vector<SurfaceSample>& vSamples)
    {
        Init(pModel, vSamples);
    }

    void Collider_SphereCloud::Init(Simulable* pModel, const vector<SurfaceSample>& vSamples)
    {
        this->Init(pModel->Geometries()[0].get(), vSamples);
        this->m_pModel = dynamic_cast<Simulable*>(pModel);
    }

    void Collider_SphereCloud::Init(Geometry* pGeom, const vector<SurfaceSample>& vSamples)
    {
    ...
    const vector<Collider_SphereCloud::Point>& Collider_SphereCloud::Points() const
    {
        return m_vPoints;
    }
}