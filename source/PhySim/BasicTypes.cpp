...
namespace PhySim
{
	...
	PtrS<AMatrixSd::Block> AMatrixSd::CreateBlock(int row, int col, int N, int M)
	{
		...
		for (int i = 0; i < pBlock->m_N; ++i)
			for (int j = 0; j < pBlock->m_M; ++j)
				pBlock->m_mPointers(i, j) = this->m_coeffMap[IntPair(pBlock->m_row + i, pBlock->m_col + j)];

		return pBlock;
	}

	bool AMatrixSd::AddStaticBlock(PtrS<AMatrixSd::Block> pBlock, const MatrixXd& mBlock)
	{
	...
	bool AMatrixSd::AddStaticBlock(int row, int col, const MatrixXd& mBlock)  //, bool assembleHessian
	{
		//if (assembleHessian)
		//{
		//	if (row >= col)
		...
		//				this->m_vvalueTripletsStatic.push_back(Triplet<double>(row + i, col + j, mBlock(i, j)));
		//	}
		//}
		//else
			if (row >= col)
			...
			else
			{
				for (int i = 0; i < mBlock.rows(); ++i)
					for (int j = 0; j < mBlock.cols(); ++j)
						this->m_vvalueTripletsStatic.push_back(Triplet<double>(col + j, row + i, mBlock(i, j)));

			}

		//// Should I use fast assembly?
		...
		//	//if (this->m_assemblyCount == (int) this->m_vassemblyBlocks.size())
		//	//{
		//	//	// The block has not been created: create it

		//	//	if (row >= col)
		//	//		this->m_vassemblyBlocks.push_back(this->CreateBlock(row, col, mBlock.rows(), mBlock.cols()));
		//	//	else this->m_vassemblyBlocks.push_back(this->CreateBlock(col, row, mBlock.cols(), mBlock.rows()));
		//	//}
		...
		//	//if (row >= col)
		//	//{
		//	//	// Check block (only for debugging)
		//	//	assert(this->m_vassemblyBlocks[m_assemblyCount]->m_row == row);
		//	//	assert(this->m_vassemblyBlocks[m_assemblyCount]->m_col == col);
		//	//	assert(this->m_vassemblyBlocks[m_assemblyCount]->m_N == mBlock.rows());
		//	//	assert(this->m_vassemblyBlocks[m_assemblyCount]->m_M == mBlock.cols());

		//	//	this->m_vassemblyBlocks[m_assemblyCount]->Add(mBlock);
		//	//}
		//	//else
		//	//{
		//	//	// Check block (only for debugging)
		//	//	assert(this->m_vassemblyBlocks[m_assemblyCount]->m_row == col);
		//	//	assert(this->m_vassemblyBlocks[m_assemblyCount]->m_col == row);
		//	//	assert(this->m_vassemblyBlocks[m_assemblyCount]->m_N == mBlock.cols());
		//	//	assert(this->m_vassemblyBlocks[m_assemblyCount]->m_M == mBlock.rows());

		//	//	this->m_vassemblyBlocks[m_assemblyCount]->Add(mBlock.adjoint());
		//	//}

		//	//// Increment block count
		//	//this->m_assemblyCount++;
		//}
	...
	bool AMatrixSd::AddDynamicBlock(int row, int col, const MatrixXd& mBlock)
	{
		...
		if (row >= col)
		{
			for (int i = 0; i < mBlock.rows(); ++i)
				for (int j = 0; j < mBlock.cols(); ++j)
					this->m_vvalueTripletsDynamic.push_back(Triplet<double>(row + i, col + j, mBlock(i, j)));
		}
		else
		{
			for (int i = 0; i < mBlock.rows(); ++i)
				for (int j = 0; j < mBlock.cols(); ++j)
					this->m_vvalueTripletsDynamic.push_back(Triplet<double>(col + j, row + i, mBlock(i, j)));
		}
	...
	void AMatrixSd::EndAssembly()
	{
		...
		if (!this->IsFast())
		{
			...
			this->setFromTriplets(
				this->m_vvalueTripletsStatic.begin(),
		...
		if (this->m_vvalueTripletsDynamic.size() != 0)
		{
			...
			mdynamicPart.setFromTriplets(
				this->m_vvalueTripletsDynamic.begin(),
