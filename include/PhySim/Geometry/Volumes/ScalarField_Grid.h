...
namespace PhySim
{
    ...
    public:
        ...
        ScalarField_Grid(const Vector3i& vShape, const Vector3d& vOrigin, const Vector3d& vDelta)
        {
    ...
    public:
        ...
        {
            assert(vShape.minCoeff() >= 1 && "Invalid dimensions for grid. The shape must be greater or equal than 1 in all dimensions!");

            m_tGrid.resize(vShape[0], vShape[1], vShape[2]);
        }

        void Setup(const Vector3i& vShape, const Vector3d& vOrigin, const Vector3d& vDelta)
        {
            assert(vShape.minCoeff() >= 1 && "Invalid dimensions for grid. The shape must be greater or equal than 1 in all dimensions!");
    ...
    public:
        ...
        {
            return { int(m_tGrid.dimension(0)), int(m_tGrid.dimension(1)), int(m_tGrid.dimension(2)) };
        }
    ...
    public:
        Scalar Sample(const Vector3d& vPoint) const override
        {
            ...
            {
                Vector3d vboxMax = m_vOrigin + this->Shape().cast<Real>().cwiseProduct(m_vDelta);
                ...
                const Vector3d vCoord = (vPoint - m_vOrigin).cwiseQuotient(m_vDelta) - Vector3d::Constant(0.5);
                const Vector3i vMinIndex = ClampIndex(vCoord.cast<int>());
                const Vector3i vMaxIndex = ClampIndex(vCoord.cast<int>() + Vector3i::Ones());
                const Vector3s vUpWeights = vCoord.array().floor().matrix().cast<Scalar>();
        ...
        Vector3s Gradient(const Vector3d& vPoint) const override
        {
            ...
            // If the underlying type is not numeric, we don't know how to compute the gradient.
            // Only act on types we know that will produce a meaningful gradient.
            if (isNumericType)
            {
                const Vector3i vShape = Shape();
                ...
                for (int i = 0; i < 3; ++i)
                {
                    // If shape along the i-th axis is 1, it's impossible to compute a gradient.
                    if (vShape[i] <= 1)
                    {
                        vGrad[i] = Scalar(0);
                        continue;
                    ...
                    Vector3i vFwdIndex = vIndex; vFwdIndex[i] += 1;
                    Vector3i vBwdIndex = vIndex; vBwdIndex[i] -= 1;
                    if (vFwdIndex[i] < 0) vFwdIndex[i] = 0;
                    if (vBwdIndex[i] >= vShape[i]) vBwdIndex[i] = vShape[i] - 1;

                    // Obtain values for differentiation.
                    const Scalar fwdValue = vFwdIndex[i] == vIndex[i] ? value : At(vFwdIndex);
                    const Scalar bwdValue = vBwdIndex[i] == vIndex[i] ? value : At(vBwdIndex);

                    // Compute differences.
                    vGrad[i] = fwdValue - bwdValue;
                    if (isFPointType) vGrad[i] /= Scalar(m_vDelta[i] * (vFwdIndex[i] - vBwdIndex[i]));
                }
            }
    ...
    public:
        ...
        Scalar& operator[](const Vector3i& vIndex)
        {
            return At(vIndex);
        }

        const Scalar& operator[](const Vector3i& vIndex) const
        {
            return At(vIndex);
        }
        ...
        {
            const Vector3i vShape = Shape();
            Vector3i vIndex = ((vPoint - m_vOrigin).cwiseQuotient(m_vDelta) - Vector3d::Constant(0.5)).cast<int>();

            if (clamp == true) 
                vIndex = vIndex.cwiseMax(Vector3i::Zero()).cwiseMin(vShape - Vector3i::Ones());
        ...
        {
            return (vIndex.cast<Real>() + Vector3d::Constant(0.5)).cwiseProduct(m_vDelta) + m_vOrigin;
        }
    ...
    private:
        ...
        {
            const Vector3i vShape = Shape();
            return vIndex.cwiseMin(vShape - Vector3i::Ones()).cwiseMax(Vector3i::Zero());
        }
