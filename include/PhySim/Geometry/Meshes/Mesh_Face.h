...
namespace PhySim
{
	...
	public:
		...
		Mesh_Face(const MatrixXd& mV, const MatrixXi& mF,
		...
		virtual void GetNodeRing_Nodes(const Node* pNode, vector<Node*>& vring, int D = 1);
		virtual void GetNodeRing_Edges(const Node* pNode, vector<Edge*>& vring, int D = 1);
		virtual void GetNodeRing_Faces(const Node* pNode, vector<Face*>& vring, int D = 1);
		...
		virtual Matrix2d ComputePlanarStrain(const Face* pFace, Tag s0, Tag sx, const VectorXd& vp);
		...
		virtual void GetBoundaryFaces(vector<Face*>& vbface, bool node = true) override;

		virtual void GetInternalNodes(vector<Node*>& vbnode) override;
		virtual void GetInternalEdges(vector<Edge*>& vbedge) override;
		virtual void GetInternalFaces(vector<Face*>& vbface, bool node = true) override;
