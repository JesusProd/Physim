...
namespace PhySim
{
	...
	public:
		...
		Mesh_Tetra(const MatrixXd& mV, const MatrixXi& mT,
				   Discretization D = Discretization_Tet4,
				   const vector<Tag>& vnTraits = vector<Tag>());

		void Init(const MatrixXd& mV, const MatrixXi& mT,
			      Discretization D = Discretization_Tet4,
				  const vector<Tag>& vnTraits = vector<Tag>());
		...
		virtual Cell_Tetra* GetTetra(int i) { return static_cast<Cell_Tetra*>(this->m_vcells[i]); }
