...
namespace PhySim
{
	...
	public:
		...
		{
			int fullIdx; // Index assuming a fully occupied regular grid
			bool surface; // Is this element a surface element? (has at least one surface node)
	...
	public:
		...
		Mesh_Grid(
			const Vector3d& vorigin, const Vector3d& vsize, const Vector3i& vdims, const iVector& voccup,
		...
		const iVector& MapFull2Active_Nodes() const { return this->m_vmapFull2Active_Nodes; }
		const iVector& MapFull2Active_Elems() const { return this->m_vmapFull2Active_Elems; }
		const iVector& MapActive2Full_Nodes() const { return this->m_vmapActive2Full_Nodes; }
		const iVector& MapActive2Full_Elems() const { return this->m_vmapActive2Full_Elems; }

		const map<pair<int, int>, PtrS<DEdge>>& DualEdges() { return this->m_mdualEdges; }

		GridNodeMeta& GetNodeGridMeta(int idx) { return this->m_vnodes[idx]->Trait<GridNodeMeta>(Tag::Tag_GridMeta); }
		GridElemMeta& GetElemGridMeta(int idx) { return this->m_velems[idx]->Trait<GridElemMeta>(Tag::Tag_GridMeta); }

		TopologyChanges CutMesh_Disc(Real radius, const Matrix3d& mR, const Vector3d& vt, Tag vs);
		void CollectCuts_Disc(Real radius, const Matrix3d& mR, const Vector3d& vt, Tag vs, list<PtrS<EdgeCut>>& cutList);
		PtrS<EdgeCut> CheckDualEdge_Disc(DEdge* pDualEdge, Real radius, const Matrix3d& mR, const Vector3d& vt, Tag vs);
		
		TopologyChanges CutMesh_Rect(Real length, Real height, const Matrix3d& mR, const Vector3d& vt, Tag vs);
		void CollectCuts_Rect(Real length, Real height, const Matrix3d& mR, const Vector3d& vt, Tag vs, list<PtrS<EdgeCut>>& cutList);
		PtrS<EdgeCut> CheckDualEdge_Rect(DEdge* pDualEdge, Real length, Real height, const Matrix3d& mR, const Vector3d& vt, Tag vs);

		TopologyChanges CutMesh(const list<PtrS<EdgeCut>>& lcutList);

		void CollectCutAffectedNodesAndElems(const Matrix3d& mR, const Vector3d& vt, Tag s, PtrS<EdgeCut> pCut);
		...
		{
			return TransformRegularCoordToIndex(this->m_vdims.x() + 1, this->m_vdims.y() + 1, this->m_vdims.z() + 1, coord);
		}

		Vector3i TransformRegularIndexToCoord_Node(const int& idx)
		{
			return TransformRegularIndexToCoord(this->m_vdims.x() + 1, this->m_vdims.y() + 1, this->m_vdims.z() + 1, idx);
		}

		int TransformRegularCoordToIndex_Elem(const Vector3i& coord)
		{
			return TransformRegularCoordToIndex(this->m_vdims.x(), this->m_vdims.y(), this->m_vdims.z(), coord);
		}

		Vector3i TransformRegularIndexToCoord_Elem(const int& idx)
		{
			return TransformRegularIndexToCoord(this->m_vdims.x(), this->m_vdims.y(), this->m_vdims.z(), idx);
		}

		int TransformRegularCoordToIndex(int dimX, int dimY, int dimZ, const Vector3i& cor)
		{
			return (dimY*dimZ)*cor.x() + (dimZ)*cor.y() + cor.z();
		}

		Vector3i TransformRegularIndexToCoord(int dimX, int dimY, int dimZ, const int& idx)
		{
			int i = idx / (dimY*dimZ);
