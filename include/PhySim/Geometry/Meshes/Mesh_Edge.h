...
namespace PhySim
{
	...
	public:
		...
		Mesh_Edge(const MatrixXd& mV, const MatrixXi& mE, Discretization D = Discretization_Edge2, const vector<Tag>& vnodeTraits = vector<Tag>());
		void Init(const MatrixXd& mV, const MatrixXi& mE, Discretization D = Discretization_Edge2, const vector<Tag>& vnodeTraits = vector<Tag>());
		virtual ~Mesh_Edge(void);

		static void Merge(PtrS<Mesh_Edge>& pMesh, const vector<PtrS<Mesh_Edge>>& vmeshes, Tag trait, Real tol);
		...
		virtual const vector<Connection*>& Connections() const { return this->m_vconns; }

		virtual Edge* GetEdge(int i) { return static_cast<Edge*>(this->m_velems[i]); }

		virtual int GetNodeConnection(int nodeID) const { return this->m_vnode2connMap[nodeID]; }
		virtual bool IsConnectionNode(int nodeID) const { return this->m_vnode2connMap[nodeID] != -1; }
