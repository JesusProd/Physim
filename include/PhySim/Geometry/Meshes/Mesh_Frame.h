...
class Mesh_Frame : public Mesh_Edge {
 public:
  ...
  void Merge(PtrS<Mesh_Frame>& pMesh,
  ...
  virtual Frame3d& GetFrame(int i, Tag s) {
    return this->GetEdge(i)->Traits().Frame3d(s);
  }

  virtual void GetRod(Edge* pEdge, vector<Edge*>& vrod) const;
  virtual void ParallalelTransportForward(Tag s,
                                          const Frame3d& F,
                                          vector<Edge*> vrod);
  virtual void ParallalelTransportBackward(Tag s,
                                           const Frame3d& F,
                                           vector<Edge*> vrod);

  virtual void SetRefFrames_TwistFreeRods(Tag s);
  virtual void SetRefFrames_FromConnectionNormals(Tag s,
                                                  const vector<Vector3d>& vn,
                                                  bool twistFree);
  virtual void SetRefFrames_FromRotatedRefFrames(Tag sourceTrait,
                                                 Tag targetTrait,
                                                 bool twistFree);
