...
namespace PhySim
{
	...
	public:
		...
		static Mesh_GridCoarsen* CreateSubCuboid(double sizeX, double sizeY, double sizeZ,
												 Vector2d ranX, Vector2d ranY, Vector2d ranZ,
												 int coarseElements, int coarseningLevel,
												 vector<Tag> vnTraits = vector<Tag>());

		static Mesh_GridCoarsen* CreateFullCuboid(double sizeX, double sizeY, double sizeZ,
												  int coarseElements, int coarseningLevel,
												  vector<Tag> vnTraits = vector<Tag>());

		static Mesh_GridCoarsen* CreateSphere(const Vector3d& center, double radius,
											  int coarseElements, int coarseningLevel, 
											  vector<Tag> vnTraits = vector<Tag>());
		...
		FineNodeMeta& GetFineNodeMeta(int idx) { return this->m_pFineMesh->Nodes()[idx]->Trait<FineNodeMeta>(Tag::Tag_FineMeta); }
		FineElemMeta& GetFineElemMeta(int idx) { return this->m_pFineMesh->Elems()[idx]->Trait<FineElemMeta>(Tag::Tag_FineMeta); }
		CoarNodeMeta& GetCoarNodeMeta(int idx) { return this->Nodes()[idx]->Trait<CoarNodeMeta>(Tag::Tag_CoarMeta); }
		CoarElemMeta& GetCoarElemMeta(int idx) { return this->Elems()[idx]->Trait<CoarElemMeta>(Tag::Tag_CoarMeta); }

		virtual void MapGlobalIndexToSubmeshIndex_Elem(int globalIdx, int& coarIdx, int& submeshIdx)
		{
			Vector3i globalCoord = this->m_pFineMesh->TransformRegularIndexToCoord_Elem(globalIdx);
			...
			submeshIdx = this->TransformRegularCoordToIndex(this->m_vcoarsening.x(),
													 this->m_vcoarsening.y(),
													 this->m_vcoarsening.z(),
													 submeshCoord);
		}

		virtual void MapSubmeshIndexToGlobalIndex_Elem(int coarIdx, int submeshIdx, int& globalIdx)
		{
			Vector3i submeshCoord = this->TransformRegularIndexToCoord(this->m_vcoarsening.x(),
															    this->m_vcoarsening.y(),
															    this->m_vcoarsening.z(),
															    submeshIdx);
			Vector3i coarCoord = this->TransformRegularIndexToCoord_Elem(coarIdx);
			Vector3i globalCoord = Vector3i(coarCoord.x()*this->m_vcoarsening.x() + submeshCoord.x(),
											coarCoord.y()*this->m_vcoarsening.y() + submeshCoord.y(),
											coarCoord.z()*this->m_vcoarsening.z() + submeshCoord.z());
			globalIdx = this->m_pFineMesh->TransformRegularCoordToIndex_Elem(globalCoord);
		}
		...
		virtual void RecomputeCoarse(list<Poly*>& laffected, TopologyChanges& changes);

		virtual void ListConnectedOrphanElements(Poly* pRoot, list<Poly*>& lfine);
		virtual Cell_Hexa* CreateCoarseElement(const Cell_Hexa* pOld, const list<Poly*>& lchildren);

		virtual PtrS<Mesh_Hexa> GetFineSubmesh(int coarElemIdx);
		virtual void GetFineSubmesh(int coarElemIdx, MatrixXd& mN, MatrixXi& mE, Tag nodeTrait);
		virtual void GetOversamplingSubmesh(vector<int> vcoarElemIdx, MatrixXd& mN, MatrixXi& mE, Tag nodeTrait);
		virtual void GetSharedNodes(int coarElemIdx, int elemSide, vector<SharedNodes>& vshared);
