...
class IInterpolation {
 public:
  ...
  virtual void Interpolate(const MatrixXd& mV,
                           const MatrixXd& mP,
                           MatrixXd& mI) const = 0;
};
...
class Interpolation : public IInterpolation {
 public:
  ...
  virtual void Compute_Weights(const VectorXd& vp,
                               VectorXd& vW) const override = 0;
  virtual void Compute_Partial(const VectorXd& vp,
                               MatrixXd& mB) const override = 0;
...
class Interpolation_Linear3 : public Interpolation {
 ...
 public:
  ...
  virtual bool IsValidParametric(const VectorXd& vp) const override {
    ...
    if (vp(0) < 0 || vp(0) > 1 || vp(1) < 0 || vp(1) > 1 || vp(0) + vp(1) > 1)
      return false;
...
class Interpolation_Bilinear : public Interpolation {
 ...
 public:
  ...
  virtual bool IsValidParametric(const VectorXd& vp) const override {
    ...
    if (vp(0) < -1 || vp(0) > 1 || vp(1) < -1 || vp(1) > 1)
      return false;
...
class Interpolation_Linear4 : public Interpolation {
 ...
 public:
  ...
  virtual bool IsValidParametric(const VectorXd& vp) const override {
    ...
    if (vp(0) < 0 || vp(0) > 1 || vp(1) < 0 || vp(1) > 1 || vp(2) < 0 ||
        vp(2) > 1 || vp(0) + vp(1) + vp(2) > 1)
      return false;
...
class Interpolation_Trilinear : public Interpolation {
 ...
 public:
  ...
  virtual bool IsValidParametric(const VectorXd& vp) const override {
    ...
    if (vp(0) < -1 || vp(0) > 1 || vp(1) < -1 || vp(1) > 1 || vp(1) < -1 ||
        vp(1) > 1)
      return false;
...
class Interpolation_Chen : public Interpolation_Trilinear {
 protected:
  vector<vector<Matrix3d>>
      m_vmNij;  // This matrix vectors store the precomputed tensor weights used
