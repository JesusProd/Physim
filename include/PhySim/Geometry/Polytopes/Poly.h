...
namespace PhySim
{
	...
	public:
		...
		virtual Real VolumeBasis(Tag s = Tag_Position_X) const override { return this->VolumeSpace(s); };
		...
		*/
		...
		virtual PtrS<IShapeFunction> GetShapeFunction() const { return this->m_pSFunction; }
		virtual void SetShapeFunction(PtrS<IShapeFunction> pSF) { this->m_pSFunction = pSF; }
		...
		*/
		...
		virtual VectorXd InterpolateDeformation(const VectorXd& vpoint, Tag s0, Tag sx) const;
		...
		* @param s The space in which node position is interpolated.
		*/
		virtual PtrS<Embedding> ComputeEmbedding(const Vector3d& vx, Tag s = Tag_Position_X) override;
		...
		* @param s The space in which node position is interpolated.
		*/
		virtual PtrS<Embedding> ComputeProjection(const Vector3d& vx, Tag s = Tag_Position_X) override;
