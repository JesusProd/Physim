 ...
 *
 * Common interface for all FEM quadrature rules of the form U = sum_i w_i
 * U(p_i), for some parametric point p. The class provides information on the
 * dimensionality of the basis, i.e. dim(p), as well as the number and value of
 * points and weights.
 */
class IQuadrature {
 public:
...
class Quadrature : public IQuadrature {
 ...
 public:
  ...
  inline virtual const vector<Real>& Compute_Values() const {
    return this->m_vweights;
  }
  inline virtual const vector<VectorXd>& Points() const {
...
class Quadrature_Tet4 : public Quadrature {
 ...
 public:
  Quadrature_Tet4() {
    ...
    this->m_vpoints[0] =
        Vector3d(0.5854101966249685, 0.1381966011250105, 0.1381966011250105);
    this->m_vpoints[1] =
        Vector3d(0.1381966011250105, 0.1381966011250105, 0.1381966011250105);
    this->m_vpoints[2] =
        Vector3d(0.1381966011250105, 0.1381966011250105, 0.5854101966249685);
    this->m_vpoints[3] =
        Vector3d(0.1381966011250105, 0.5854101966249685, 0.1381966011250105);
...
class Quadrature_Tet8 : public Quadrature {
 ...
 public:
  Quadrature_Tet8() {
    ...
    this->m_vpoints[0] =
        Vector3d(0.009437387888358, 0.035220811090087, 0.166666666666667);
    this->m_vpoints[1] =
        Vector3d(0.035220811090087, 0.009437387888358, 0.166666666666667);
    this->m_vpoints[2] =
        Vector3d(0.035220811090087, 0.131445856471988, 0.044658198978444);
    this->m_vpoints[3] =
        Vector3d(0.131445856471988, 0.035220811090087, 0.044658198978444);
    this->m_vpoints[4] =
        Vector3d(0.035220810850163, 0.131445855576580, 0.622008467032738);
    this->m_vpoints[5] =
        Vector3d(0.131445855576580, 0.035220810850163, 0.622008467032738);
    this->m_vpoints[6] =
        Vector3d(0.131445855576580, 0.490562611456158, 0.166666666666667);
    this->m_vpoints[7] =
        Vector3d(0.490562611456158, 0.131445855576580, 0.166666666666667);
...
class Quadrature_Tet11 : public Quadrature {
 ...
 public:
  Quadrature_Tet11() {
    ...
    this->m_vpoints[0] =
        Vector3d(0.250000000000000, 0.250000000000000, 0.250000000000000);
    this->m_vpoints[1] =
        Vector3d(0.785714285714286, 0.071428571428571, 0.071428571428571);
    this->m_vpoints[2] =
        Vector3d(0.071428571428571, 0.071428571428571, 0.071428571428571);
    this->m_vpoints[3] =
        Vector3d(0.071428571428571, 0.071428571428571, 0.785714285714286);
    this->m_vpoints[4] =
        Vector3d(0.071428571428571, 0.785714285714286, 0.071428571428571);
    this->m_vpoints[5] =
        Vector3d(0.100596423833201, 0.399403576166799, 0.399403576166799);
    this->m_vpoints[6] =
        Vector3d(0.399403576166799, 0.100596423833201, 0.399403576166799);
    this->m_vpoints[7] =
        Vector3d(0.399403576166799, 0.399403576166799, 0.100596423833201);
    this->m_vpoints[8] =
        Vector3d(0.399403576166799, 0.100596423833201, 0.100596423833201);
    this->m_vpoints[9] =
        Vector3d(0.100596423833201, 0.399403576166799, 0.100596423833201);
    this->m_vpoints[10] =
        Vector3d(0.100596423833201, 0.100596423833201, 0.399403576166799);
...
class Quadrature_Gauss : public Quadrature {
 public:
  Quadrature_Gauss(int dimBasis, int numPoints) {
    ...
    MathUtils::gaussQuadrature(numPoints, -1, 1, this->m_vpoints,
                               this->m_vweights);
  }
};
    ...
    for (size_t i = 0; i < numPoints; i++)
      for (size_t j = 0; j < numPoints; j++)
        for (size_t k = 0; k < numPoints; k++) {
          auto center = Vector3d((2 * i + 1) * halflength - 1,
                                 (2 * j + 1) * halflength - 1,
                                 (2 * k + 1) * halflength - 1);
          for (auto q : vq) {
 ...
 public:
  Quadrature_Uniform(int numPoints) {
    ...
    for (size_t i = 0; i < numPoints; i++)
      for (size_t j = 0; j < numPoints; j++)
        for (size_t k = 0; k < numPoints; k++) {
          this->m_vpoints.push_back(Vector3d((2 * i + 1) * halflength - 1,
                                             (2 * j + 1) * halflength - 1,
                                             (2 * k + 1) * halflength - 1));
