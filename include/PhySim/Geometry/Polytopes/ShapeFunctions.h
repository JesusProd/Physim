 ...
 *
 * Common interface for all polytope shape (interpolation) function of the form
 * x = sum_i N_i(p), for some parametetric point p. The class provides
 * information on the dimensionality of the basis, i.e. dim(p), and the space,
 * i.e. dim(x). It must also provide information on the number of interpolated
 * control points, i.e., 4 in the case of a linear tetrahedron, together with
 * the interpolation weight N_i(p) and partial derivatives, dN_i(p)/dp.
 */
class IShapeFunction {
 public:
  ...
  virtual void InterpolateDeformation(const Poly* pPoly,
                                      Tag trait0,
                                      Tag traitX,
                                      const MatrixXd& mP,
                                      MatrixXd& mI) const = 0;
  virtual void InterpolateDeformation(const MatrixXd& mN0,
                                      const MatrixXd& mNx,
                                      const MatrixXd& mP,
                                      MatrixXd& mI) const = 0;
  virtual void InterpolateValue(const Poly* pPoly,
                                Tag trait,
                                const MatrixXd& mP,
                                MatrixXd& mI) const = 0;
  virtual void InterpolateValue(const MatrixXd& mN,
                                const MatrixXd& mP,
                                MatrixXd& mI) const = 0;
  ...
  virtual void UpdateKinematicsAtSamples(
      PtrS<IDeformationData> pData) const = 0;
  virtual void UpdateDeformationAtSamples(
      PtrS<IDeformationData> pData) const = 0;
};
...
class ShapeFunction : public IShapeFunction {
 public:
  struct DeformationData : public IDeformationData {
   public:
    ...
    virtual void SetPoly(const Poly* pPoly) override {
    ...
    virtual void SetPoints(const vector<VectorXd>& vp) override {
      this->m_vpoints = vp;
    }
    virtual const vector<MatrixXd>& Get_J() const override {
      return this->m_vmJ;
    };
    virtual const vector<MatrixXd>& Get_F() const override {
      return this->m_vmF;
    };
    virtual const vector<MatrixXd>& Get_DFDx() const override {
      return this->m_vmDFDx;
    };
  ...
  virtual void InitDeformationAtSamples(
      PtrS<IDeformationData> pData) const override {}
  virtual void UpdateKinematicsAtSamples(
      PtrS<IDeformationData> pData) const override {}
  virtual void UpdateDeformationAtSamples(
      PtrS<IDeformationData> pData) const override {}
 ...
 protected:
  virtual void Compute_MatrixX(
      MatrixXd& mNX,
      PtrS<IShapeFunction::IDeformationData> pDataParent) const;
  virtual void Compute_Matrix0(
      MatrixXd& mN0,
      PtrS<IShapeFunction::IDeformationData> pDataParent) const;
};
...
class ShapeFunction_LERP : public ShapeFunction {
 ...
 public:
  ...
  virtual void InitDeformationAtSamples(
      PtrS<IShapeFunction::IDeformationData> pDataParent) const override;
  virtual void UpdateDeformationAtSamples(
      PtrS<IShapeFunction::IDeformationData> pDataParent) const override;
...
class ShapeFunction_Node : public ShapeFunction_LERP {
 ...
 public:
  ...
  virtual void Compute_Partial(const VectorXd& vp,
                               MatrixXd& mB) const override {
    mB = MatrixXd();
  }
};
...
class ShapeFunction_Chen : public ShapeFunction_Hex8 {
 ...
 public:
  struct DeformationData : public ShapeFunction::DeformationData {
   public:
    PtrS<ShapeFunction::DeformationData>
        m_pRotationSamples;  // Central sample used to compute element rotation
  ...
  ShapeFunction_Chen(const vector<vector<Matrix3d>>& vmnij,
                     const VectorXi& vCoarsening);

  virtual ~ShapeFunction_Chen(){};

  virtual PtrS<IShapeFunction::IDeformationData> CreateDeformationData() const {
    PtrS<DeformationData> pDefData =
        PtrS<DeformationData>(new DeformationData());
    pDefData->m_pRotationSamples =
        dynamic_pointer_cast<ShapeFunction::DeformationData>(
            ShapeFunction_Hex8::CreateDeformationData());
 ...
 protected:
  virtual void Compute_F(MatrixXd& mF,
                         PtrS<IShapeFunction::IDeformationData> pDataParent,
                         int k) const;
  virtual void Compute_DFDx(MatrixXd& mDFDx,
                            PtrS<IShapeFunction::IDeformationData> pDataParent,
                            int k) const;
