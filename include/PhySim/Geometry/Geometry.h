...
class Geometry {
 ...
 public:
  ...
  template <class T>
  void GetNodesTrait(vector<T>& vvalues, Tag s) const;
  template <class T>
  void SetNodesTrait(const vector<T>& vvalues, Tag s);

  template <class T>
  void GetElemsTrait(vector<T>& vvalues, Tag s) const;
  template <class T>
  ...
  template <class T>
  inline T& Trait(Tag s) {
    return this->m_traits.Trait<T>(s);
  }
  template <class T>
  inline const T& Trait(Tag s) const {
    return this->m_traits.Trait<T>(s);
  }

  virtual PtrS<Embedding> ComputeProjection(const Vector3d& vx,
                                            Tag s = Tag_Position_X);
  ...
  virtual void EmbedNodes(vector<Node*>& toEmbed,
                          const Tag& trait = Tag_Position_X);
  virtual void EmbedNodes(vector<Node*>& toEmbed,
                          const VectorXi& vsel = VectorXi());
