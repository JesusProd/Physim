...
class TraitSet {
 private:
  ...
  class TraitCaseT : public TraitCase {
   public:
    ...
    virtual TraitCase* Clone() const {
      return new TraitCaseT<T>(*static_cast<T*>(pValue));
    }
  };
 ...
 public:
  ...
  TraitSet(const TraitSet& toCopy) { this->CloneTraits(toCopy); }

  virtual ~TraitSet() { this->ClearTraits(); }
  ...
  int& Int(Tag s) { return *static_cast<int*>(m_mData.at(s)->pValue); }

  double& Double(Tag s) { return *static_cast<double*>(m_mData.at(s)->pValue); }
  ...
  void CloneTraits(const TraitSet& toCopy) {
    ...
    for (; itCur != itEnd; ++itCur) {
      this->m_mData.insert(
          pair<Tag, TraitCase*>(itCur->first, itCur->second->Clone()));
    }
  }
  ...
  bool AddTrait(Tag s, const int& value) { return AddTrait<int>(s, value); }
