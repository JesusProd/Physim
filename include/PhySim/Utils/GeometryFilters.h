...
namespace PhySim
{
	...
	public:
		...
		GeometryFilter_IndexNode(const iVector& vindices)
		{
			m_vindices = vindices;
		}
	...
	public:
		...
		GeometryFilter_IndexElem(const iVector& vindices)
		{
			m_vindices = vindices;
		}
	...
	public:
		GeometryFilter_BoundaryFace(bool useNodes)
		{
			this->m_useNodes = useNodes;
		}
	...
	public:
		GeometryFilter_BoxNodes(const VectorXd& vminCor, const VectorXd& vmaxCor, Tag sema)
		{
		...
		virtual vector<Geometry*> Filter(PtrS<Geometry> pGeom) const override
		{
			...
			for (int i = 0; i < pMesh->Nodes().size(); ++i)
			{
				...
				if (vx.x() >= this->m_vminCorner.x() && vx.x() <= this->m_vmaxCorner.x() &&
					vx.y() >= this->m_vminCorner.y() && vx.y() <= this->m_vmaxCorner.y() &&
	...
	public:
		GeometryFilter_BoxElems(const VectorXd& vminCor, const VectorXd& vmaxCor, Tag sema)
		{
			this->m_vminCorner = vminCor;
			this->m_vmaxCorner = vmaxCor;
			this->m_sema = sema;
	...
	public:
		...
		GeometryFilter_PlaneDistNode(Tag space, const Vector3d& vp, const Vector3d& vn, const Vector2d& vrange)
		{
			this->m_space = space;
			this->m_vp = vp;
			this->m_vn = vn;
    ...
    public:
        ...
        GeometryFilter_HullNodes(Tag space, const MatrixXd& vX, const MatrixXi& vF)
        {
            ...
            //    Cost: O(X·F).
            // B) Build the convex hull of the mesh and see if any vertices were dropped.
            //    Cost(X·log(X)).
            // Probably not worth implementing any of these though =)

            // Build per-face planes used to test points if the points are within the convex
            // hull.
        ...
        virtual vector<Geometry*> Filter(PtrS<Geometry> pGeom) const override
        {
            ...
            PtrS<Mesh> pMesh = dynamic_pointer_cast<Mesh>(pGeom);
            if (pMesh == nullptr) return vFiltered;
            ...
            //#pragma omp parallel 
            {
                //vector<Geometry*>& vThreadFiltered = vPerThreadFiltered[omp_get_thread_num()];
            ...
            //    vFiltered.insert(vFiltered.end(), vThreadFiltered.begin(), vThreadFiltered.end());
