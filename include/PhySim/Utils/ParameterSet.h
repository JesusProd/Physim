...
class ParameterSet {
 public:
  ...
  virtual ~ParameterSet() { this->m_params.clear(); }

  inline virtual const map<string, Real>& ModelParameters() const {
  ...
  inline Real GetParameter(const string& name) const {
    if (HasParameter(name))
      ...
      throw PhySim::exception("Parameter not found");
  }
  ...
  inline void ClearParameters() { this->m_params.clear(); }
