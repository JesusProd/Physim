...
namespace PhySim
{
	...
	public:
		...
		VectorAnimator_LerpValues(const vector<VectorXd>& vvalIni, const vector<VectorXd>& vvalEnd)
		{
			// Check
		...
		virtual string Name() const override { return VectorAnimator_LerpValues::NAME; }
	...
	public:
		...
		VectorAnimator_LerpT2D(const Vector2d& vT, const Vector2d& vR, const Vector2d& vS, const vector<VectorXd>& vvalIni = vector<VectorXd>())
		{
			this->m_vT = vT;
			this->m_vR = vR;
			this->m_vS = vS;
	...
	public:
		...
		VectorAnimator_LerpT3D(const Vector3d& vT, const Vector3d& vR, const Vector3d& vS, const vector<VectorXd>& vvalIni = vector<VectorXd>())
		{
		...
		VectorAnimator_LerpT3D(const Matrix4d& mT, const vector<VectorXd>& vvalIni = vector<VectorXd>())
		{
	...
	public:
		...
		VectorAnimator_SequenceT2D(const vector<Matrix3d>& vmT, const vector<int>& vSteps, const vector<VectorXd>& vvalIni)
		{
		...
		virtual string Name() const override { return VectorAnimator_SequenceT2D::NAME; }
		...
		virtual void Evaluate(Real t, vector<VectorXd>& vval) const override
		{
			...
			for (int i = 0; i < (int)this->m_vmT[i].size(); ++i)
			{
				if (t <= this->m_times[i])
				{
					...
					for (int i = 0; i < numP; ++i)
						vval[i] = mTlerp.block(0, 3, 3, 1) + mTlerp.block(0, 0, 3, 3)*vval[i];
	...
	public:
		...
		VectorAnimator_SequenceT3D(const vector<Matrix4d>& vmT, const vector<int>& vSteps, const vector<VectorXd>& vvalIni)
		{
                  IOUtils::logTrace(
                      Verbosity::V2_SoftDebug,
			...
			for (int i = 0; i < (int)vmT.size(); ++i)
			{
				...
				this->m_vQ[i] = GeometryUtils::rotationMatrixToAxisAngle(mR);
                                IOUtils::logTrace(
                                    Verbosity::V3_HardDebug,
                                    "\n[TRACE] Translation: (%f,%f,%f)",
                                    m_vT[i](0), m_vT[i](1), m_vT[i](2));
                                IOUtils::logTrace(
                                    Verbosity::V3_HardDebug,
                                    "\n[TRACE] Rotation: (%f,%f,%f)",
                                    m_vQ[i](0), m_vQ[i](1), m_vQ[i](2));
		...
		virtual string Name() const override { return VectorAnimator_SequenceT3D::NAME; }
		...
		virtual void Evaluate(Real t, vector<VectorXd>& vval) const override
		{
			...
			for (int i = 0; i < (int) this->m_vT.size(); ++i)
			{
                          IOUtils::logTrace(
                              Verbosity::V3_HardDebug,
                              "\n[TRACE] Transformation of the sequence: %d",
                              i);
				...
				if (t < this->m_times[i])
				{
					...
					if (i == 0)
						ti = t / this->m_times[i];
					else ti = (t - this->m_times[i - 1]) / (this->m_times[i] - this->m_times[i - 1]);
