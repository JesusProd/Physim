...
namespace PhySim
{
	...
	public:
		...
		//template <typename T> bool LoadMatrix(const xml_node& ele, Matrix<T, -1, -1>& mat);
		//template <typename T> bool LoadVector(const xml_node& ele, Matrix<T, -1, 1>& mat);
		...
		bool LoadSimulableVector(const xml_attr& att, Simulable* pModel, VectorXd& vx);

		bool LoadDiscretization(const xml_node& ele, Discretization& dis);

		template <typename T> bool LoadDistributed(const xml_node& ele, PtrS<T>& pEle);
		...
		template <typename T> bool LoadDistribution(const xml_node& ele, PtrS<IDomainDistribution<PtrS<T>>>& pDist);

		//bool LoadMaterialSampler(const xml_node& ele, int dimSpace, int dimBasis, vector<PtrS<IMaterialModel>>& vpModel, vector<ParameterSet>& vParam);

		bool LoadMesh_Surface(const xml_node& ele, MatrixXd& mV, MatrixXi& mF, MatrixXd& mN);

		bool LoadMesh_HexVolume(const xml_node& ele, MatrixXd& mV, MatrixXi& mF, MatrixXi& mT);
		bool LoadMesh_TetVolume(const xml_node& ele, MatrixXd& mV, MatrixXi& mF, MatrixXi& mT);
		...
		bool LoadOptimProblem(const xml_node& ele, ISimulable* pModel, PtrS<OptimProblem>& pProblem);

		bool LoadOptimSolver(const xml_node& ele, IOptimProblem* pProblem, PtrS<OptimSolver>& pSolver);

		bool LoadBoundary(const xml_node& ele, Simulable* pModel, vector<PtrS<IBCondition>>& vpBC);

		bool LoadBoundary(const xml_node& ele, Simulable* pModel, PtrS<IBCondition>& pBC);
		...
		void InvalidNodeValue(const xml_node& ele, const string& expected, bool warning);
		void InvalidAttriValue(const xml_attr& att, const string& expected, bool warning);
