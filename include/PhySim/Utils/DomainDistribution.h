...
namespace PhySim
{
	...
	public:
		...
		virtual T& GetValueAtDomainPoint(int eleIdx, const VectorXd& vpoint = VectorXd::Zero(3)) = 0;
	};
	...
	public:
		...
		DomainDistribution_Constant(const T& value)
		{
			this->m_value = value;
		}
		virtual ~DomainDistribution_Constant() {}

		virtual T& GetValueAtDomainPoint(int eleIdx, const VectorXd& vpoint = VectorXd::Zero(3)) { return this->m_value; };
	};
	...
	public:
		...
		virtual T& GetValueAtDomainPoint(int eleIdx, const VectorXd& vpoint = VectorXd::Zero(3)) { return this->m_vvalue[m_vmapEle[eleIdx]]; };
	...
	public:
		...
		DomainDistribution_Subelement(const vector<T>& vvalue, const MatrixXi& mmapEle, const VectorXi& vCoarsening)
		{
			this->m_vvalue = vvalue;
		...
		virtual T& GetValueAtDomainPoint(int eleIdx, const VectorXd& vpoint = VectorXd::Zero(3)) 
		{ 
	...
	private:
		int ChooseSubelement(const VectorXd& vp) const
		{
			...
			return x * this->m_vCoarsening.y()*this->m_vCoarsening.z() + y * this->m_vCoarsening.z() + z;
		};
	...
	public:
		...
		DomainDistribution_Procedural(PtrS<ISpatialDistribution<T>> pFunction, const vector<Poly*>& vPolys, Tag layer)
		{
		...
		virtual T& GetValueAtDomainPoint(int eleIdx, const VectorXd& vpoint = VectorXd()) 
		{ 
