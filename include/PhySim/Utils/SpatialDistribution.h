...
class SpatialDistribution : public ISpatialDistribution<T> {
 ...
 public:
  SpatialDistribution(
      const vector<PtrS<IDeformer>>& vdeformer = vector<PtrS<IDeformer>>()) {
...
class SpatialDistribution_BisectionX : public SpatialDistribution<T> {
 ...
 public:
  SpatialDistribution_BisectionX(
      Real barrier,
      T& value0,
      T& value1,
      const vector<PtrS<IDeformer>>& vdeformer = vector<PtrS<IDeformer>>())
      : SpatialDistribution<T>(vdeformer) {
  ...
  virtual T& GetValueAtSpatialPointInternal(const VectorXd& vx) override {
    ...
    if (vx.x() > m_zero) {
      if (s % 2 == 0)
        ...
        return m_value1;  // Odd stripe
    } else {
      if (s % 2 == 0)
        return m_value1;  // Even stripe
      else
        return m_value0;  // Odd stripe
    }
  }
};
...
class SpatialDistribution_Grid : public SpatialDistribution<T> {
 ...
 public:
  SpatialDistribution_Grid(
      const VectorXd& vzero,
      const VectorXd& vwidth,
      T& value0,
      T& value1,
      const vector<PtrS<IDeformer>>& vdeformer = vector<PtrS<IDeformer>>())
      : SpatialDistribution<T>(vdeformer) {
