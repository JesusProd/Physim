...
namespace PhySim
{
	...
	public:
		DerivativeAssembler(bool isDynamic = false)
		{
		...
		void AssemblePreprocessedGradient(const VectorXd& vgradient, AVectorXd& vglobalGradient);
		void AssemblePreprocessedHessian(const MatrixXd& mHessian, AMatrixSd& vglobalHessian);

		void PropagateGradient(const VectorXd& vgradient, const vector<IDoFSet*>& vDoF, int layer);
		void AssembleGradient(AVectorXd& vglobalGradient);

		void PropagateHessian(const MatrixXd& mHessian, const vector<IDoFSet*>& vDoF, int layer);
		void AssembleHessian(AMatrixSd& vglobalHessian);

		void PropagateAndAssembleGradient(const VectorXd& vgradient, const vector<IDoFSet*>& vDoF, AVectorXd& vglobalGradient);
		void PropagateAndAssembleHessian(const MatrixXd& mHessian, const vector<IDoFSet*>& vDoF, AMatrixSd& mglobalHessian);


		void AssembleJacobian(const MatrixXd& mJacobian, const vector<IDoFSet*>& vDoF, AMatrixSd& mglobalJacobian);
