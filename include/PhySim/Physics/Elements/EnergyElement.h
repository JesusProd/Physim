...
class EnergyElement : public IEnergyElement {
 ...
 public:
  ...
  inline const VectorXd& GetLocalGradient() const override {
    return this->m_vgradient;
  }
  inline const MatrixXd& GetLocalHessian() const override {
    return this->m_mHessian;
  }

  virtual void PreprocessAssembly(int layer) override;
  virtual void AssemblePreprocessedGradient(
      AVectorXd& vglobalGradient) override;
  ...
  virtual void PropagateAndAssemble_Gradient(
      AVectorXd& vglobalGradient) override;
  virtual void PropagateAndAssemble_Hessian(AMatrixSd& mglobalMatrix) override;

  virtual int GetHessianSize() const override {
    return (int)pow(this->GetSupportSize(), 2);
  };

  inline virtual const Real& GetIntegrationVolume() const override {
    return this->m_intVolume;
  }
  inline virtual void SetIntegrationVolume(const Real& iv) override {
    this->m_intVolume = iv;
  }
