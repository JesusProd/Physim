...
class EnergyElement_SixDJoint : public EnergyElement {
 protected:
  PtrS<KEleRigidBody3D> m_pRBp,
      m_pRBc;  // the two rigids that the joint is defined between them, parent
               // and child
  ...
  Vector3d m_rp, m_rc, m_jointCenter, m_xNodep,
      m_xNodec;  // vector of rigid cm to joint point, and the coordinates of
                 // the node thatbelongs to the joint

  // PtrS<ParameterSet> m_pMaterial;

 public:
  EnergyElement_SixDJoint(Simulable* pModel,
