...
class Simulable_SpineModel : public Simulable_Composite {
 ...
 public:
  ...
  virtual void Setup() override {
    ...
    m_rigidBodyModel =
        dynamic_pointer_cast<Simulable_RB>(this->m_pOptions->m_vpSimulables[0]);
    m_softBodyModel = dynamic_pointer_cast<Simulable_FEM>(
        this->m_pOptions->m_vpSimulables[1]);
    ...
    for (int i = 0; i < rbEnergyElements.size(); ++i) {
      this->m_rbEnergyElements[i] =
          dynamic_pointer_cast<EnergyElement_SixDJoint>(rbEnergyElements[i]);
    }
    ...
    m_defaultParams << 1.1 * scale1, 7.8 * scale1, 1. * scale1, 1.52 * scale2,
        1.53 * scale2, 1.48 * scale2,  /// thoracic
        2.45 * scale1, 17.2 * scale1, 2.45 * scale1, 1.43 * scale2,
        4.98 * scale2, 1.49 * scale2,                  /// lumbar
  ...
  void SetParameters(const VectorXd& vp) {
    VectorXd helpVecInitializeSpineJoints(
        6);  /// just to add to a vector the 4 parameters under optimization and
    ...
    for (int i = 0; i < 17; i++) {
      ...
      if (i < 11) {  /// 11 because of number of thoracic joints
        K = (helpVecInitializeSpineJoints.cwiseProduct(
                 m_defaultParams.segment(0, 6)))
                .asDiagonal();
      } else if (i < 17) {  //// nbVertVertJoints
        K = (helpVecInitializeSpineJoints.cwiseProduct(
                 m_defaultParams.segment(6, 6)))
                .asDiagonal();  /// same parameter but different initialization
      }
      // else if (i < 17 + 24 ) {
    ...
    // PtrS<DomainDistribution_Constant<ParameterSet>> pParamsFem =
    // static_pointer_cast<DomainDistribution_Constant<ParameterSet>>(paramsFem2);
    // ParameterSet& paramsFem = pParamsFem->Value();
    ...
    // pMatParam->InitFromYoungPoisson(m_defaultParams[24] * vp[4],
    // m_defaultParams[25] * vp[5], 1000);
    /////////////end  young poisson        ///////////

    (*paramsFem)[ParameterSet::Param_Lame2] = m_defaultParams[24] * vp[4];
    (*paramsFem)[ParameterSet::Param_Lame1] = m_defaultParams[25] * vp[5];
    PtrS<ParameterSet> pMatParam = std::make_shared<ParameterSet>();
    pMatParam->InitFromLameParameter(m_defaultParams[25] * vp[5],
                                     m_defaultParams[24] * vp[4], 1000);
    this->m_softBodyModel->SetupOptions().m_pMatParams.reset(
    ...
    // this->m_softBodyModel->SetupOptions().m_pMatParams->GetValueAtDomainPoint(0);
    ...
    // this->m_softBodyModel->SetupOptions().m_pMatParams.reset(new
  ...
  void GetParameters(VectorXd& vp) const {
    ...
    // because of the specific parametrization, I can consrtuct vp with only
    // these values
    MatrixXd K = m_rbEnergyElements[0]->GetParameters();
    helpVecInitializeSpineJoints =
        K.diagonal().cwiseQuotient(m_defaultParams.segment(0, 6));

    // K = m_rbEnergyElements[18]->GetParameters();
    // vp.segment(6, 6) = K.diagonal().cwiseQuotient(m_defaultParams.segment(12,
    // 6));

    // K = m_rbEnergyElements[42]->GetParameters();
    // vp.segment(12, 6) =
    // K.diagonal().cwiseQuotient(m_defaultParams.segment(18, 6));
  ...
  void getRigidVectors(vector<Matrix3d>& rotMat,
                       vector<Vector3d>& posVec,
                       vector<int>& indces) {
    vector<PtrS<KinematicsEle>> eles = m_pTree->RootElements();
    for (int i = 0; i < eles.size(); i++) {
      KEleRigidBody3D* pEle =
          dynamic_cast<KEleRigidBody3D*>(m_pTree->RootElements()[i].get());
  ...
  void Compute_DFDp_FiniteDifferences(MatrixSd& DFdp, int nParams) {
    ...
    std::cout << " Parameters inside Compute_DFDp_FiniteDifferences, after "
                 "getting them = "
              << vp << std::endl;
    AVectorXd fplus, fminus;
    this->GetGradient(fplus);
    std::cout << " parameters vp inside Compute_DFDp_FiniteDifferences " << vp
              << std::endl;
    ...
    for (int i = 0; i < vp.size(); i++) {
      // std::cout << " gradient inside Compute_DFDp_FiniteDifferences, before
      // set parameters " << fplus.norm() << std::endl;
      double h = 1e-3 * vp[i];
      vp[i] += h;
      this->SetParameters(vp);
      this->GetParameters(vp);
      std::cout << " Parameters inside Compute_DFDp_FiniteDifferences, after "
                   "setting them for first time = "
                << vp << std::endl;
      this->GetGradient(fplus);
      // std::cout << " gradient inside Compute_DFDp_FiniteDifferences, fplus "
      // << fplus.norm() << std::endl;
      ...
      std::cout << " gradient inside Compute_DFDp_FiniteDifferences, fminus "
                << fminus.norm() << std::endl;
      DFDp_dense.block(0, i, fplus.size(), 1) = -(fplus - fminus) / (2 * h);
      vp[i] += h;
      this->SetParameters(vp);
      // std::cout<< "DFDp_dense.block(0, i="<< i <<" , 18 * 6, 1) " <<
      // DFDp_dense.block(0, i, 18 * 6, 1) << std::endl;
    }
