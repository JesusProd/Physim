...
class Simulable_FEM : public Simulable_Mesh {
 ...
 public:
  ...
  virtual PtrS<IDomainDistribution<PtrS<ParameterSet>>>&
  MatParamsDistribution() {
    return this->SetupOptions().m_pMatParams;
  }
  virtual PtrS<IDomainDistribution<PtrS<IMaterialModel>>>&
  MatModelsDistribution() {
    return this->SetupOptions().m_pMatModels;
  }
  virtual PtrS<IDomainDistribution<PtrS<IQuadrature>>>&
  QuadraturesDistribution() {
    return this->SetupOptions().m_pQuadratures;
  }
  virtual PtrS<IDomainDistribution<PtrS<IShapeFunction>>>&
  ShapeFunctionsDistribution() {
    return this->SetupOptions().m_pShapeFunctions;
  }

  virtual void GetRestStrains(vector<MatrixXd>& vE0) const;
  virtual void SetRestStrains(const vector<MatrixXd>& vE0);

  virtual const vector<EnergyElement_FEM*>& GetEnergyElements_FEM() const {
    return this->m_venergyEle_fem;
  }
