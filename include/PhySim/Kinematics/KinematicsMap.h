...
namespace PhySim
{
	...
	public:
		KinematicsMap();
		KinematicsMap(Simulable* pModel, const vector<PtrS<KinematicsEle>>& vIn, const vector<PtrS<KinematicsEle>>& vOut);
		virtual void Init(Simulable* pModel, const vector<PtrS<KinematicsEle>>& vIn, const vector<PtrS<KinematicsEle>>& vOut);
		...
		virtual void GatherMapChain(int owner, int idxOut, vector<vector<PtrS<KMappingBlock>>>& vpmBlocks, vector<KinematicsEle*>& vpKinem);

		virtual void MapDerivatives(int owner, int idxOut, const MatrixXd& mDeri, vector<KinematicsEle*>& vpEles, vector<MatrixXd>& vmBlocks);

		virtual void MapValue(int idxIn, const VectorXd& vpIn, int idxOut, VectorXd& vpOut) = 0;
