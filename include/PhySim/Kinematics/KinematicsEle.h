...
namespace PhySim
{
	...
	struct KMappingBlock_Variable : public KMappingBlock
	{
		...
		int iIn;
		KinematicsMap* map;
		KMappingBlock_Variable(int iOut, int iIn, KinematicsMap* map) : KMappingBlock()
		{
			this->iOut = iOut;
			this->iIn = iIn;
			this->map = map;
	...
	public:
		...
		KinematicsEle() : DoFSet()
		{
			this->Initialize();
		}

		KinematicsEle(Simulable* pModel) : DoFSet(pModel)
		{
			this->Initialize();
		}

		KinematicsEle(Simulable* pModel, Geometry* pGeom, int numDim) : DoFSet(pModel, pGeom, numDim)
		{ 
			this->Initialize();
		}
		...
		VectorXd GetValue() const override
		{
			return this->GetPositionX();
		}

		void SetValue(const VectorXd& vv) override
		{
			this->SetPositionX(vv);
		}
		...
		void GatherMapChain(int owner, vector<vector<PtrS<KMappingBlock>>>& vpmBlocks, vector<KinematicsEle*>& vpKinem);

		void MapDerivatives(int owner, const MatrixXd& mBIn, vector<KinematicsEle*>& vpKOut, vector<MatrixXd>& vmBOut);
